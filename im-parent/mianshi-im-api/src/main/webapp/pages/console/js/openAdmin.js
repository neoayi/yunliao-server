layui.use(['form','layer','laydate','table','laytpl'],function(){
    var form = layui.form,
	    ...
	    var tableIns = table.render({
	      ...
	      ,groups: 7
	      ,cols: [[ //表头
	           {field: 'accountId', title: '申请用户Id',width:'12%'}
	          ,{field: 'appName', title: '应用名称',width:'12%'}
	          ,{field: 'appIntroduction', title: '应用简介',  width:'12%'}
	          ,{field: 'appUrl', title: '应用官网', width:'12%'}
	          ,{field: 'iosAppId', title: 'Bundle ID', width:'12%'}
	          ,{field: 'iosBataAppId', title: '测试版本Bundle ID', width:'12%'}
	          ,{field: 'createTime',title:'申请时间', width:'12%',templet: function(d){
	          		return UI.getLocalTime(d.createTime);
	          }}
	          ,{fixed: 'right',title:"操作", width:'10%', align:'left', toolbar: '#openAppListBar'}
	        ]]
			,done:function(res, curr, count){
				checkRequst(res);

				//权限判断
				var arr=['appUser-detail','appUser-delete','appUser-applicationList'];
	...
	appDetail:function(id){
		Common.invoke({
			...
			success:function(result){
				...
				$("#appId").empty();
				$("#appId").append(Common.filterHtmlData(Common.isUndefined(result.data.appId)));
				$("#appSecret").empty();
				$("#appSecret").append(Common.filterHtmlData(Common.isUndefined(result.data.appSecret)));
				$("#iosAppId").empty();
				...
				$("#androidAppId").empty();
				$("#androidAppId").append(Common.filterHtmlData(Common.isUndefined(result.data.androidAppId)));
				$("#androidDownloadUrl").empty();
				$("#androidDownloadUrl").append(Common.filterHtmlData(Common.isUndefined(result.data.androidDownloadUrl)));
				$("#androidSign").empty();
				$("#androidSign").append(Common.filterHtmlData(Common.isUndefined(result.data.androidSign)));
				$("#payCallBackUrl").empty();
				$("#payCallBackUrl").append(Common.filterHtmlData(Common.isUndefined(result.data.payCallBackUrl)));
	...
	applicationList:function(index){
		...
		var tableIns = layui.table.render({
			...
			,cols: [[ //表头
				...
				,{field: 'appIntroduction', title: '应用简介', width:150}
				,{field: 'appUrl', title: '应用官网', width:150}
				,{field: 'iosAppId', title: 'Bundle ID', width:150}
				,{field: 'iosBataAppId', title: '测试版本Bundle ID', width:150}
				,{field: 'createTime', title: '申请时间', width:200,templet: function(d){
						return UI.getLocalTime(d.createTime);
				}}
