var page=0;
var lock=0;
var userIds = new Array();
var toUserIds = new Array();
var messageIds = new Array();
var nickName;
var userId;
var regeditPhoneOrName;
var currentPageIndex;// 当前页码数
var currentCount;// 当前总数
var exportStartTime;// 过滤导出手机号注册的开始时间
var exportEndTime;// 过滤导出手机号注册的结束时间
var exportKeyword;// 过滤导出手机号使用的关键字
var exportOnline;// 过滤导出手机号使用的在线状态
var tableData;//存放分页数据
var InviteCodeContest;//当前邀请码状态
var pattern = new RegExp("[`~!@#$^&*()=|{}':;',\\[\\].<>/?~！@#￥……&*（）——|{}【】‘；：”“'。，、？%]");
var chains = /[^\x00-\xff]/ig; //中文符号正则表达式
var update_user = 0;
var datas = {};
var operationFlag = false;
var mapApi = "https://api.map.baidu.com/staticimage?width=320&height=240&&zoom=15&markers=";
var chooseDeleteCount = 0;
var array_alphabetic_regular = /^[0-9a-zA-Z]*$/;
layui.config({
    base : "/pages/common/dropdown/"
}).use(['form','layer','laydate','table','laytpl','dropdown'],function(){
    var form = layui.form,
        layer = parent.layer === undefined ? layui.layer : top.layer,
        $ = layui.jquery,
        laydate = layui.laydate,
        laytpl = layui.laytpl,
        table = layui.table,
        dropdown = layui.dropdown;

    // 监听下拉菜单点击
    dropdown.onFilter('test1', function (event) {
        if (event == "autoCreateUser") {
            User.autoCreateUser()
        }else if (event == "exportRandUser"){
            export_rand_user();
        }else if (event == "exportExcelPhone"){
            User.exportExcelPhone();
        }
    })

    //根据系统邀请码类型移除对应邀请码的按钮
    if(localStorage.getItem("registerInviteCode")!=2){
        $(".create_populer_inviteCode").remove();
    }else{
        $(".btn_create_register_InviteCode").remove();
    }
    //用户列表
    var tableInUser = table.render({
        elem: '#user_list'
        ,toolbar: '#toolbarUsers'
        ,url:request("/console/userList")
        ,id: 'user_list'
        ,page: {
            layout: [ 'prev', 'page', 'next','limit', 'count', 'skip'] //自定义分页布局
            ,groups: 3 //只显示 1 个连续页码
            ,first: false //不显示首页
            ,last: false //不显示尾页
        }
        ,curr: 0
        ,limit:Common.limit
        ,limits:Common.limits
        ,groups: 7
        ,totalRow: true
        ,cols: [[
            {type:'checkbox',fixed:'left', totalRowText: '合计'}// 多选
            ,{field: 'userId', title: '用户Id', width:100}
            ,{field: 'nickname', title: '昵称', width:100}
            ,{field: 'isOpen', title: '是否开启端到端加密', width:180,templet:function(d){
                    return d.isOpenResetPwd == 0 ? "否" : "是" ;
                }}
            ,{field: 'telephone', title: '手机号码', width:135,templet:function(d){
                    if(0 == Config.getConfig().regeditPhoneOrName){
                        if(!Common.isNil(d.telephone))
                            return d.telephone;else return "";
                    }else{
                        return d.phone;
                    }
                }}
            ,{field: 'account', title: '通讯号', width:145}
            ,{field: 'balance', title: '账户余额', width:145, totalRow: true}
            ,{field: 'isAuth', title: '短信注册', width:110,templet: function(d){
                    return d.isAuth==1?"是":"否";
            }}
            ,{field: 'realNameCertified', title: '是否实人认证', width:135,templet: function(d){
                return d.realNameCertified==1?"是":"否";
            }}
            ,{field: 'model', title: '登录设备', width:110,templet:function(d){
                    if(null != d.loginLog){
                        var model = d.loginLog.model;
                        if(null == model || undefined == model || "" == model){
                            return "其他设备";
                        }else{
                            return model;
                        }
                    }
                    return "其他设备";

                }}
            ,{field: 'ip', title: 'IP', width:170,template(d){
                    return d.loginLog.ipAddress;
                }}
            ,{field:'createTime', title:'位置', width:80, templet: function(d){
                    if (Common.isNil(d.loc)){
                        return "";
                    }
                   return '<button onclick="User.showMap(\'' + d.loc.lng + '\' , \'' + d.loc.lat + '\')" class="layui-btn layui-btn-sm font_bule">查看</button>';
                }}
            ,{field: 'createTime', title: '注册时间', width:170,templet: function(d){
                    return UI.getLocalTime(d.createTime);
                }}
            ,{field: 'onlinestate', title: '在线状态', width:105,templet: function(d){
                    return (d.onlinestate==0?"离线":"在线");
                }}

            ,{field: 'loginLog', title: '最后上线时间', width:170,templet: function(d){
                    // console.log("log    :"+JSON.stringify(d.loginLog));
                    if(d.loginLog==undefined){
                        return "";
                    }else{
                        if(d.loginLog.loginTime==0){
                            return "";
                        }else{
                            return UI.getLocalTime(d.loginLog.loginTime);
                        }
                    }
                }}

            ,{field: 'loginLog', title: '最后离线时间', width:170,templet: function(d){
                    if(d.loginLog==undefined){
                        return "";
                    }else{
                        if(d.loginLog.offlineTime==0){
                            return "";
                        }else{
                            return UI.getLocalTime(d.loginLog.offlineTime);
                        }
                    }
                }}
            ,{fixed: 'right', width: '33%',title:"操作", align:'left', toolbar: '#userListBar'}
        ]]
        ,done:function(res, curr, count){
            checkRequst(res);
            //开启下拉菜单
            dropdown.suite();
            //权限判断
            var arr=['user-delete','user-update','user-randUser','user-recharge','user-handCash','user-bill','user-createInviteCode','user-add','user-rebot','user-lock','user-friends','user-sendMsg'];
            manage.authButton(arr);

            $(".layui-table-box").find("[data-field='isOpenResetPwd']").css("display","none");
            //获取零时保留的值
            var last_value = $("#userList_limlt").val();
            //获取当前每页大小
            var recodeLimit =  tableInUser.config.limit;
            //设置零时保留的值
            $("#userList_limlt").val(recodeLimit);
            //判断是否改变了每页大小数
            if (last_value != recodeLimit){
                // 刷新
                table.reload("user_list",{
                    url:request("/console/userList"),
                    page: {
                        curr: 1 //重新从第 1 页开始
                    }
                })
            }

            $("#operation").addClass('operationClass');

            //日期范围
            layui.laydate.render({
                elem: '#userListMsgDate'
                ,range: "~"
                ,done: function(value, date, endDate){  // choose end
                    //console.log("date callBack====>>>"+value); //得到日期生成的值，如：2017-08-18
                    var startDate = value.split("~")[0];
                    var endDate = value.split("~")[1];
                    // Count.loadGroupMsgCount(roomJId,startDate,endDate,timeUnit);
                    // 导出手机号
                    exportStartTime = startDate;
                    exportEndTime = endDate;
                    var nameKeyWord = Common.getValueForElement(".nickName");
                    if(null == nameKeyWord || ""==nameKeyWord){
                        nameKeyWord = "";
                    }
                    table.reload("user_list",{
                        url:request('/console/userList'),
                        page: {
                            curr: 1 //重新从第 1 页开始
                        },
                        where: {
                            // userId : data.userId,  //搜索的关键字
                            startDate : startDate,
                            endDate : endDate,
                            keyWord:nameKeyWord
                        }
                    })
                }
                ,max: 1
            });

            if(count==0&&lock==1){
                layer.msg("暂无数据",{"icon":2});
                renderTable();
            }
            lock=0;
            $("#userList").show();
            $(".user_table").show();
            $("#addUser").hide();
            $("#autoCreateUser").hide();
            $("#exportUser").hide();
           /* if(localStorage.getItem("role")==1 || localStorage.getItem("role")==4 || localStorage.getItem("role")==7){
                $("#checkDeleteUsersId").hide();
                $(".btn_addUser").hide();
                $(".btn_createUser").hide();
                $(".export_rand_user").hide();
                $(".export_telephone").hide();
                $(".new_registerUser").hide();
                $(".delete").hide();
                $(".update").hide();
                $(".randUser").hide();
                $(".locking").hide();
                $(".cancelLocking").hide();
                $(".recharge").hide();
                $(".bill").hide();
                $(".friends").hide();
                $(".deleteFriends").hide();
                $(".createInviteCode").hide();
                $(".handCash").hide();
            }*/
            var pageIndex = tableInUser.config.page.curr;//获取当前页码
            var resCount = res.count;// 获取table总条数
            currentCount = resCount;
            currentPageIndex = pageIndex;
            User.operation('-1');
            if (operationFlag){
                User.openOperation();
                $("#operation").html('关闭操作');
            }else{
                User.closeOperation();
                $("#operation").html('打开操作');
            }
        }


    });

    //监听表格复选框选择
    table.on('checkbox(user_list)', function(obj){
        //当前页总数据量  --> 更换表名
        var thisCount = table.cache.user_list.length;
        //每页数据量
        var pageCount = $(".layui-laypage-limits").find("option:selected").val();


        if (obj.type == 'one'){
            chooseDeleteCount = obj.checked == true ? chooseDeleteCount + 1 : chooseDeleteCount - 1;
        }else if (obj.type == 'all') {
            chooseDeleteCount = obj.checked == true ? pageCount > thisCount ? thisCount : pageCount : 0;
        }
        $(".chooseDeleteCount").html("已选择 "+chooseDeleteCount+" 项");
        chooseDeleteCount < 1 ? $(".mustDelete").hide() :$(".mustDelete").show();
    });

    $(".nickName").val('');

    //列表操作
    table.on('tool(user_list)', function(obj){
        var layEvent = obj.event,
            data = obj.data;

        if(layEvent === 'delete'){ //删除
            layer.confirm('确认要删除吗？', {
                btn : [ '确定', '取消' ]//按钮
                ,skin : "layui-ext-motif"
            }, function(index) {
                layer.close(index);
                if(data.userId<=10000){
                    return;
                }
                User.checkDeleteUsersImpl(data.userId,1);
                obj.del();
            });
        }else if(layEvent === 'update'){// 修改用户
            update_user = 1;
            if(data.userId<=10000){
                return;
            }
            User.updateUser(obj.data,obj.data.userId);

        }else if(layEvent === 'randUser'){// 重置密码
            if (data.userId <= 10000) {
                return;
            }
            User.ResetPassword(obj.data.userId,obj.data.areaCode,obj.data.phone,data.isOpenResetPwd);

        }else if(layEvent === 'locking'){// 锁定
            if(data.userId<=10000){
                return;
            }
            User.lockIng(data.userId,-1)

        }else if(layEvent === 'cancelLocking'){// 解锁
            User.lockIng(data.userId,1)

        }else if (layEvent === 'handCash') {// 手工提现
            var isclick = true;
            layer.prompt({title: '请输入提现金额', formType: 0, value: '1'}, function (money, index) {

                /*// 提现金额为两位小数
                if(0 == money){
                    layer.msg("请输入1-1000 的金额",{"icon":2});
                    return;
                }
                var pattern = /^(\d|[1-9]\d|1000)(\.\d{1,2})?$/;
                if(!pattern.test(money)){
                    layer.msg("请输入 1-1000 的金额",{"icon":2});
                    return;
                }*/
                /* if(1 != money){
                     layer.msg("每次只能提现1元",{"icon":2});
                     return;
                 }*/
                if (isclick) {
                    isclick = false;
                    Common.invoke({
                        url: request('/console/handCash'),
                        data: {
                            money: money,
                            userId: data.userId
                        },
                        successMsg: "提现成功",
                        errorMsg: "提现失败，请稍后重试",
                        success: function (result) {

                            var data = result.data; //DataSort(result.data);
                            layer.close(index); //关闭弹框
                            // 更新用户列表
                            layui.table.reload("user_list")

                        },
                        error: function (result) {
                            layer.close(index); //关闭弹框
                        }
                    });
                    //定时器
                    setTimeout(function () {
                        isclick = true;
                    }, 500);
                }
            });
        }else if(layEvent==='bill'){ //用户账单
            localStorage.setItem("currClickUser", data.userId);
            /*localStorage.setItem("currClickUser", data.userId);
            layer.open({
              title : "",
                 type: 2,
                 skin: 'layui-layer-rim', //加上边框
                 area: ['750px', '500px'], //宽高
                 content: 'userBill.html'
                 ,success: function(index, layero){
                 }
               });*/
            // 置空时间范围
            $("#userBillMsgDate").val('');
            // $(".billType").val('0');
            $(".visitPathDiv").hide();
            $(".user_btn_div").hide();
            $(".userListSearch").hide();
            $(".billInfo").show();
            $(".userBillName").val(data.nickname);
            $(".userBillName").empty();
            $(".userBillName").append( Common.getValueForElement(".userBillName") );// 用户账单中的小标题用户昵称
            //用户账单
            var tableIns = layui.table.render({
                elem: '#myBill_table'
                ,url:request("/console/userBill")+"&userId="+data.userId
                ,id: 'myBill_table'
                ,page: {
                    layout: [ 'prev', 'page', 'next','limit', 'count', 'skip'] //自定义分页布局
                    ,groups: 3 //只显示 1 个连续页码
                    ,first: false //不显示首页
                    ,last: false //不显示尾页
                }
                ,curr: 0
                ,limit:Common.limit
                ,limits:Common.limits
                ,groups: 7
                ,cols: [[ //表头
                    {field: 'tradeNo', title: '交易单号', width:200}
                    ,{field: 'money', title: '金额', width:100}
                    ,{field: 'serviceCharge', title: '手续费', width:120}
                    ,{field: 'operationAmount', title: '实际金额', width:120}
                    ,{field: 'currentBalance', title: '账户余额', width:120}
                    ,{field: 'time', title: '交易时间', width:200,templet: function(d){
                            return  UI.getLocalTime(d.time);
                        }}
                    ,{field: 'type', title: '类型', width:150,templet: function(d){
                            if (d.type==1){return "用户充值";}
                            else if(d.type==2){return "用户提现";}
                            else if(d.type==3){return "后台充值";}
                            else if(d.type==4){return "发送红包";}
                            else if(d.type==5){return "领取红包";}
                            else if(d.type==6){return "红包退款";}
                            else if(d.type==7){return "转账";}
                            else if(d.type==8){return "接受转账";}
                            else if(d.type==9){return "转账退回";}
                            else if(d.type==10){return "付款码付款";}
                            else if(d.type==11){return "付款码收款";}
                            else if(d.type==12){return "二维码收款 付款方";}
                            else if(d.type==13){return "二维码收款 收款方";}
                            else if(d.type==14){return "直播送礼物";}
                            else if(d.type==15){return "直播礼物收入";}
                            else if(d.type==16){return "后台手工提现";}
                            else if(d.type==17){return "第三方调用支付";}
                            else if(d.type==18){return "扫码手动充值";}
                            else if(d.type==60){return "商城订单收入";}
                            else if(d.type==61){return "商城订单退款";}
                            else if(d.type==62){return "商城订单付款";}
                            else if(d.type==63){return "转发商品收入";}
                            else if(d.type==64){return "团队下单奖励收入";}
                            else if(d.type==65){return "订单平台手续费支出";}
                            else if(d.type==66){return "转发分成支出";}
                            else if(d.type==67){return "团队下单奖励支出";}
                            else{return "未知";}
                        }}
                    ,{field: 'payType', title: '支付方式', width:150,templet: function(d){
                            if (d.payType==1){return "支付宝支付";}
                            else if(d.payType==2){return "微信支付";}
                            else if(d.payType==3){return "余额支付";}
                            else if(d.payType==4){return "系统支付";}
                            else{return "其他方式支付"}
                        }}
                    ,{fixed: 'right', width: 250,title:"操作", align:'left', toolbar: '#billOperation'}

                ]]
                ,done:function(res, curr, count){
                    checkRequst(res);
                    $(".user_table").hide();
                    $(".myBill").show();
                    console.log("res : "+JSON.stringify(res));

                    var totalInfo = JSON.parse(res.totalVo);
                    $(".totalRecharge").empty().text(totalInfo.totalTecharge);
                    $(".totalCash").empty().text(totalInfo.totalCash);
                    $(".totalTransfer").empty().text(totalInfo.totalTransfer);
                    $(".totalAccount").empty().text(totalInfo.totalAccount);
                    $(".sendPacket").empty().text(totalInfo.sendPacket);
                    $(".receivePacket").empty().text(totalInfo.receivePacket);
                    //日期范围
                    layui.laydate.render({
                        elem: '#userBillMsgDate'
                        ,range: "~"
                        ,done: function(value, date, endDate){  // choose end
                            //console.log("date callBack====>>>"+value); //得到日期生成的值，如：2017-08-18
                            var startDate = value.split("~")[0];
                            var endDate = value.split("~")[1];

                            layui.table.reload("myBill_table",{
                                page: {
                                    curr: 1 //重新从第 1 页开始
                                },
                                where: {
                                    // userId : data.userId,  //搜索的关键字
                                    startDate : startDate,
                                    endDate : endDate
                                }
                            })
                        }
                        ,max: 1
                    });
                }
            });
        }else if(layEvent==='recharge'){ //后台充值
            var isclick = true;
            layer.prompt({title: '请输入充值金额', formType: 0, value: '1'}, function (money, index) {
                // 充值金额（正整数）的正则校验
                /*if(!/^(?!00)(?:[0-9]{1,3}|1000)$/.test(money)){
                    layer.msg("请输入 1-1000 的整数",{"icon":2});
                    return;
                }*/
                /* if(1 != money){
                     layer.msg("每次只能充值1元",{"icon":2});
                     return;
                 }*/
                if (isclick) {
                    isclick = false;
                    Common.invoke({
                        url: request('/console/Recharge'),
                        data: {
                            money: money,
                            userId: data.userId
                        },
                        successMsg: "充值成功",
                        errorMsg: "充值失败，请稍后重试",
                        success: function (result) {

                            var data = result.data; //DataSort(result.data);
                            layer.close(index); //关闭弹框
                            // 更新用户列表
                            layui.table.reload("user_list")

                        },
                        error: function (result) {
                            layer.close(index);
                        }
                    });
                    //定时器
                    setTimeout(function () {
                        isclick = true;
                    }, 500);
                }
            });
        }else if(layEvent==='friends'){// 好友管理

            nickName = Common.filterHtmlData(data.nickname);
            userId = data.userId; //给当前操作的那一行数据对应的用户的userId 记录，后面使用
            $(".disUserName").val(nickName);
            $(".disUserName").empty();
            $(".disUserName").append( Common.getValueForElement(".disUserName"));// 好友管理中的小标题用户昵称
            var tableInsFriends = table.render({
                elem: '#myFriends_table'
                ,toolbar: '#toolbarUsersFriends'
                ,url:request("/console/friendsList") + "&userId=" + userId
                ,id: 'myFriends_table'
                ,page: {
                    layout: [ 'prev', 'page', 'next','limit', 'count', 'skip'] //自定义分页布局
                    ,groups: 3 //只显示 1 个连续页码
                    ,first: false //不显示首页
                    ,last: false //不显示尾页
                }
                ,curr: 0
                ,limit:Common.limit
                ,limits:Common.limits
                ,groups: 7
                ,cols: [[ //表头
                    {type:'checkbox',fixed:'left'}// 多选
                    ,{field: 'toUserId', title: '好友Id', width:120}
                    ,{field: 'toNickname', title: '好友昵称', width:150}
                    ,{field: 'status', title: '状态', width:120,templet : function (d) {
                            if(d.status == -1)
                                return "黑名单";
                            else if(d.status == 1)
                                return "关注"
                            else if(d.status == 2)
                                return "好友";
                        }}
                    ,{field: 'blacklist', title: '是否拉黑', width:120,templet : function (d) {
                            if(d.blacklist == 1)
                                return "是";
                            else
                                return "否";
                        }}
                    ,{field: 'isBeenBlack', title: '是否被拉黑', width:120,templet : function (d) {
                            if(d.isBeenBlack == 1)
                                return "是";
                            else
                                return "否";
                        }}
                    ,{field: 'offlineNoPushMsg', title: '是否消息免打扰',width: 150,templet : function (d) {
                            if(d.offlineNoPushMsg == 1)
                                return "是";
                            else
                                return "否";
                        }}
                    ,{field: 'createTime', title: '成为好友的时间',width: 170,templet: function(d){
                            return UI.getLocalTime(d.createTime);}}
                    ,{fixed: 'right', width: 500,title:"操作", align:'left', toolbar: '#delFriends'}
                ]]
                ,done:function(res, curr, count){
                    checkRequst(res);
                    $(".user_table").hide();
                    $("#myFriends").show();
                    $(".userListSearch").hide();
                    $(".visitPathDiv").hide();
                    var pageIndex = tableInsFriends.config.page.curr;//获取当前页码
                    var resCount = res.count;// 获取table总条数
                    currentCount = resCount;
                    currentPageIndex = pageIndex;
                }
            });

            //监听表格复选框选择
            table.on('checkbox(myFriends_table)', function(obj){
                //当前页总数据量
                var thisCount = table.cache.myFriends_table.length;
                //每页数据量
                var pageCount = $(".layui-laypage-limits").find("option:selected").val();


                if (obj.type == 'one'){
                    chooseDeleteCount = obj.checked == true ? chooseDeleteCount + 1 : chooseDeleteCount - 1;
                }else if (obj.type == 'all') {
                    chooseDeleteCount = obj.checked == true ? pageCount > thisCount ? thisCount : pageCount : 0;
                }
                $(".chooseDeleteCount").html("已选择 "+chooseDeleteCount+" 项")
                chooseDeleteCount < 1 ? $(".mustDelete").hide() :$(".mustDelete").show();
            });

        }else if(layEvent==='sendMsg'){
            // 发送消息
            User.sendUserMsg(data.userId);
        }else if(layEvent == 'realNameCertified'){
            //实人认证信息
            User.realNameCertified(data.userId);
        }else if(layEvent==='inviteCode'){  //用户邀请码

            nickName = Common.filterHtmlData(data.nickname);
            userId = data.userId; //给当前操作的那一行数据对应的用户的userId 记录，后面使用
            $(".disUserName").val(nickName);
            $(".disUserName").empty();
            $(".disUserName").append(Common.getValueForElement(".disUserName"));// 好友管理中的小标题用户昵称
            //邀请码列表
            var tableIns = table.render({

                elem: '#myInviteCode_table'
                ,url: request("/console/inviteCodeList")+"&userId="+data.userId
                ,id: 'invite_code_table'
                ,page: {
                    layout: [ 'prev', 'page', 'next','limit', 'count', 'skip'] //自定义分页布局
                    ,groups: 3 //只显示 1 个连续页码
                    ,first: false //不显示首页
                    ,last: false //不显示尾页
                }
                ,curr: 0
                ,limit:10
                ,limits:[10,20,30,40,50,100,1000,10000,100000]
                ,groups: 7
                ,cols: [[ //表头
                    {field: 'inviteCode', title: '邀请码',width:100}
                    ,{field: 'userId', title: '所属用户Id',width:110}
                    ,{field: 'status', title: '状态',width:100,templet:function(d){
                            if(d.status==0){
                                return "未使用";
                            }else{
                                return "已使用";
                            }
                        }}
                    ,{field: 'totalTimes', title: '类型',width:200,templet:function(d){
                            if(d.totalTimes==-1){
                                return "推广型(一码多用)";
                            }else if(d.totalTimes==1){
                                return "邀请注册型(一码一用)";
                            }
                        }}
                    ,{field: 'createTime',title:'生成时间', width:180,templet: function(d){
                            return Common.formatDate(d.createTime,"yyyy-MM-dd hh:mm:ss",1);
                        }}
                    ,{field: 'usedTimes', title: '使用次数',width:100}
                    ,{field: 'lastuseTime',title:'最后使用时间', width:180,templet: function(d){
                            if(d.lastuseTime==0){
                                return '--';
                            }else{
                                return Common.formatDate(d.lastuseTime,"yyyy-MM-dd hh:mm:ss",1);
                            }

                        }}
                    ,{fixed: 'right', width: 200,title:"操作", align:'left', toolbar: '#inviteCodeBar'}
                ]]
                ,done:function(res, curr, count){
                    tableData = table.cache.invite_code_table;
                    checkRequst(res);
                    $(".user_table").hide();
                    $("#myInviteCode").show();
                    $(".user_btn_div").hide();

                    var pageIndex = tableIns.config.page.curr;//获取当前页码
                    var resCount = res.count;// 获取table总条数
                    currentCount = resCount;
                    currentPageIndex = pageIndex;
                }
            });

        }

    });

/*    $(function () {
        //获取资源信息
        $.ajax({
            type:"POST",
            url:request("/console/querySecurityRole1"),
            data:{},
            dataType:'json',
            async:false,
            success:function(result){
                datas = result.data;
            }
        })
    })*/

    // 好友管理
    table.on('tool(myFriends_table)', function(obj){
        var layEvent = obj.event,
            data = obj.data;
        // console.log(data);
        if(layEvent === 'deleteFriends'){// 删除好友
            // User.deleteFrends(data.userId,data.toUserId,obj);
            if(-1 == data.status)
                return;
            User.checkDeleteUsersFriendsImpl(data.userId,data.toUserId,1);
        }else if(layEvent==='chatRecord'){// 聊天记录
             var tableInsChatRecord = table.render({
                 elem: '#friendsChatRecord_table'
                ,toolbar: '#toolbarUsersChatRecord'
                ,url:request("/console/friendsChatRecord")+"&userId="+data.userId+"&toUserId="+data.toUserId
                ,id: 'friendsChatRecord_table'
                ,page: {
                     layout: [ 'prev', 'page', 'next','limit', 'count', 'skip'] //自定义分页布局
                     ,groups: 3 //只显示 1 个连续页码
                     ,first: false //不显示首页
                     ,last: false //不显示尾页
                 }
                ,curr: 0
                ,limit:Common.limit
                ,limits:Common.limits
                ,groups: 7
                ,cols: [[ //表头
                    {type:'checkbox',fixed:'left'}// 多选
                    ,{field: 'senderJid', title: '用户Id', width:120, templet : function (d){
                        return d.sender_jid == 0 ? d.sender : d.sender_jid;
                         }}
                    ,{field: 'sender_nickname', title: '发送者', width:120}
                    ,{field: 'receiverJid', title: '好友Id', width:120, templet:function (d){
                        return d.receiver_jid == 0 ? d.receiver : d.receiver_jid;
                         }}
                    ,{field: 'receiver_nickname', title: '接收者', width:150}
                    ,{field: 'contentType', title: '消息类型', width:150,templet : function (d) {
                            return Common.msgType(d.contentType);
                        }}
                    ,{field: 'timeSend', title: '发送时间',width: 170,templet: function(d){
                            return UI.getLocalTime(d.timeSend/1000);}}
                    ,{field: 'content', title: '消息内容', width:250,templet:function (d) {
                        if (!Common.isNil(d.content)) {
                            if (localStorage.getItem("role") == 6) {
                                if (1 == d.isEncrypt) {
                                    // des解密
                                    var desContent = Common.decryptMsg(d.content, d.messageId, d.timeSend);
                                    if (desContent.search("https") != -1 || desContent.search("http") != -1) {
                                        var link = "<a target='_blank' href=\"" + desContent + "\">" + desContent + "</a>";
                                        return link;
                                    } else {
                                        return desContent;
                                    }
                                } else if (2 == d.isEncrypt) {
                                    // aes解密
                                    let testContent = Common.decryptAESMessage(d.messageId, d.content);
                                    console.log("aes testContent :" + testContent);
                                    return testContent;
                                }
                            }
                            var text = (Object.prototype.toString.call(d.content) === '[object Object]' ? JSON.stringify(d.content) : d.content)
                            try {
                                if (text.search("https") != -1 || text.search("http") != -1) {
                                    var link = "<a target='_blank' href=\"" + text + "\">" + text + "</a>";
                                    return link;
                                } else {
                                    return text;
                                }
                            } catch (e) {
                                return text;
                            }
                        } else {
                            return "";
                        }
                    }}
                    ,{field: 'deleteTime', title: '保留截止时间',width: 170,templet: function(d){
                        if(0 < d.deleteTime)
                            return UI.getLocalTime(d.deleteTime);
                        else return"";
                    }}
                    ,{fixed: 'right', width: 200,title:"操作", align:'right', toolbar: '#delChartRecord'}
                ]]
                ,done:function(res, curr, count){
                    console.log(res.data)
                    checkRequst(res);
                    $(".user_table").hide();
                    $("#myFriends").hide();
                    $("#friendsChatRecord").show();
                    var pageIndex = tableInsChatRecord.config.page.curr;//获取当前页码
                    var resCount = res.count;// 获取table总条数
                    currentCount = resCount;
                    currentPageIndex = pageIndex;
                }
            });
        }else if(layEvent==='joinBlacklist'){ //加入黑名单
            User.joinMoveBlacklist(data.userId,data.toUserId,0);
        }else if(layEvent==='moveBlacklist'){ //移除黑名单
            User.joinMoveBlacklist(data.userId,data.toUserId,1);
        }else if(layEvent == 'chatRecordWindow'){
            var contestData;
            $.ajax({
                type:"POST",
                url:request("/console/friendsChatRecord")+"&userId="+data.userId+"&toUserId="+data.toUserId,
                data:{
                    page:0,
                    limit:99999999
                },
                dataType:'json',
                async:false,
                success:function(result){
                    contestData = result.data;
                }
            })

            //聊天记录
            //打开聊天面板
            layui.layer.open({
                title:"",
                skin: 'layui-ext-motif',
                type: 1,
                shade: false,
                area: ['1000px', '800px'],
                shadeClose: true, //点击遮罩关闭
                content: $("#mp_chatPanel"),
                cancel: function(index, layero){ //关闭聊天面板后执行
                    layui.layer.close(index)
                    //清空页面上的聊天消息
                    $("#mp_chatPanel #messageContainer").empty();
                },
                success : function(layero,index){  //弹窗打开成功后的回调
                    $(".nano").nanoScroller();

                    chat.createMsg(contestData);

                    setTimeout(function(){ //将滚动条移动到最下方
                        $(".nano").nanoScroller();//刷新滚动条
                        chat.scrollToEnd(); //滚动到底部
                    },400);
                }
            });
        }else if (layEvent == 'destroyFriendMessage'){
            Common.invoke({
                url : request('/console/destroyFriendMessage'),
                data : {
                    userId:data.userId,
                    toUserId:data.toUserId
                },
                successMsg : "销毁单聊双方聊天记录成功",
                errorMsg :  "网络异常，请稍后重试",
                success : function(result) {
                    layui.table.reload("myFriends_table")
                },
                error : function(result) {
                }
            });
        }
    });

    // 删除好友间的聊天记录
    table.on('tool(friendsChatRecord_table)', function(obj){
        var layEvent = obj.event,
            data = obj.data;
        console.log(data);
        if(layEvent === 'deletechatRecord'){
            // User.delFriendsChatRecord(data.messageId,obj);
            User.toolbarUsersChatRecordImpl(data.messageId,1);
        }
    });

    // 邀请码操作
    table.on('tool(myInviteCode_table)', function(obj){
        var layEvent = obj.event,
            data = obj.data;
        console.log(data);
        console.log(obj.event);
        if(layEvent === 'del_inviteCode'){
            User.deleteInviteCode(data.userId,data.id);
            obj.del();
        } else
        if(layEvent === 'updateInviteCode'){
            layui.layer.open({
                type: 1,
                title: '请输入邀请码',
                area: ['300px', '180px'], // 弹窗大小
                content: `
                <div style="padding: 20px;">
                    <input type="text" id="userInput" class="layui-input" maxlength="6" placeholder="请输入6位数字" style="1px solid  #ff6700;" />
                    <button id="confirmButton" style="margin-top: 20px;width: 35%;float: right; color: #3da2ff;background-color: white;border: 1px rgba(0, 0, 0, .5) solid;
                    ">确定</button>
                </div>
            `,
                cancel: function(index, layero){ //关闭聊天面板后执行
                    layui.layer.close(index)
                },
                success: function (layero, index) {
                    $('#userInput').on('input', function () {
                        var value = $(this).val();
                        // 只允许输入数字
                        $(this).val(value.replace(/\D/g, ''));
                    });
                    // 确定按钮点击事件
                    $('#confirmButton').click(function () {
                        var inputValue = $('#userInput').val(); // 获取输入框的值
                        if(inputValue === ""){
                            layer.msg('请输入内容！');
                            return
                        }
                        if(inputValue.length < 6){
                            layer.msg('请输入6位数字！');
                            return
                        }


                        // if (inputValue === "") {
                        //     layer.msg('请输入内容！');
                        // } else {
                            Common.invoke({
                                url : request('/console/updateInviteCode'),
                                data : {
                                    userId:data.userId,
                                    code:inputValue
                                },
                                successMsg : "修改成功",
                                errorMsg :  "网络异常，请稍后重试",
                                success : function(result) {
                                    layui.layer.close(index);
                                    layui.table.reload("user_list")
                                },
                                error : function(result) {
                                }
                            });


                            // $.ajax({
                            //     url: '/console/updateInviteCode?userId=' + data.userId + "&code=" + inputValue, // 替换为你的后端接口
                            //     type: 'GET',
                            //     success: function (response) {
                            //         if (response.resultCode == 1) {
                            //
                            //         } else {
                            //             layer.msg(response.resultMsg, {icon: 2});
                            //         }
                            //         //
                            //         // $('#qrcodeImage').attr('src', response.data.url);
                            //         // $("#id-secretKey").val(response.data.secretKey);
                            //     },
                            //     error: function () {
                            //         alert('失败');
                            //     }
                            // });
                            // 关闭弹窗

                       // }
                    });
                }
            });


        }
    });

    form.on('switch(status)', function(obj){
        //根据业务判断是开启还是关闭
        var state = obj.elem.checked?1:-1;
        //方法二取数据 （根据索引table.cache里面的行数据）
        var index  = obj.othis.parents('tr').attr("data-index");
        var inviteCode = tableData[index].inviteCode;
        var userId = tableData[index].userId;
        User.prohibitInviteCode(userId,inviteCode,state);
    });


    // 账单明细
    table.on('tool(myBill_table)', function(obj){
        var layEvent = obj.event,
            data = obj.data;
        if(layEvent === 'billDetails'){
            console.log("账单 明细  "+JSON.stringify(data));
            var tableBillInfo;
            // 红包相关
            if(4 == data.type || 5 == data.type || 6 == data.type){
                if(Common.isNil(data.redPacketId)){
                    layer.msg("暂无明细",{"icon":2});
                    return;
                }
                tableBillInfo = table.render({
                    elem: '#myBill_table_Info'
                    ,url:request("/console/redPacketList")+"&redPacketId="+data.redPacketId
                    ,id: 'myBill_table_Info'
                    ,page: {
                        layout: [ 'prev', 'page', 'next','limit', 'count', 'skip'] //自定义分页布局
                        ,groups: 3 //只显示 1 个连续页码
                        ,first: false //不显示首页
                        ,last: false //不显示尾页
                    }
                    ,curr: 0
                    ,limit:Common.limit
                    ,limits:Common.limits
                    ,groups: 7
                    ,cols: [[ //表头
                        {field: 'userId', title: '用户Id',width:150}
                        ,{field: 'userName', title: '昵称',width:180}
                        ,{field: 'money', title: '红包金额', width:120}
                        ,{field: 'count', title: '红包个数', width:120}
                        ,{field: 'status', title: '状态', width:150, templet : function (d) {
                                var statusMsg;
                                (d.status == 1 ? statusMsg="发出" : (d.status == 2) ? statusMsg = "已领完" :(d.status == -1) ? statusMsg = "已退款" : (d.status == 3) ? statusMsg = "未领完退款" : "")
                                return statusMsg;
                            }}
                        ,{field: 'sendTime',title:'发送时间',width:195,templet: function(d){
                                return UI.getLocalTime(d.sendTime);
                            }}
                        ,{fixed: 'right', width: 250,title:"操作", align:'left', toolbar: '#redPageListBar'}
                    ]]
                    ,done:function(res, curr, count){
                        if(0==count){
                            layer.msg("暂无红包记录明细",{"icon":2});
                            return;
                        }
                        checkRequst(res);
                        $(".user_table").hide();
                        $(".myBill").hide();
                        $("#myBillInfo").show();
                    }
                });

            }else if(1 == data.type || 3 == data.type || 2 == data.type || 10 == data.type || 11 == data.type ||
                12 == data.type || 13 == data.type || 16 == data.type || 18 == data.type || 19 == data.type){
                // 充值和提现相关处理 // 10  12
                var titleMsg,moneyInfo,timeInfo;
                if(1 == data.type || 3 == data.type || 18 == data.type){
                    titleMsg = "充值记录Id";
                    moneyInfo = "充值金额";
                    timeInfo = "充值时间";
                }else if(2 == data.type || 16 == data.type){
                    titleMsg = "提现记录Id";
                    moneyInfo = "提现金额";
                    timeInfo = "提现时间";
                }else if(10 == data.type || 12 == data.type){
                    titleMsg = "付款记录Id";
                    moneyInfo = "付款金额";
                    timeInfo = "付款时间";
                }else if(11 == data.type || 13 == data.type){
                    titleMsg = "收款记录Id";
                    moneyInfo = "收款金额";
                    timeInfo = "收款时间";
                }
                tableBillInfo = table.render({
                    elem: '#myBill_table_Info'
                    ,url:request("/console/consumeRecordInfo")+"&tradeNo="+data.tradeNo
                    ,id: 'myBill_table_Info'
                    ,page: {
                        layout: [ 'prev', 'page', 'next','limit', 'count', 'skip'] //自定义分页布局
                        ,groups: 3 //只显示 1 个连续页码
                        ,first: false //不显示首页
                        ,last: false //不显示尾页
                    }
                    ,curr: 0
                    ,limit:Common.limit
                    ,limits:Common.limits
                    ,groups: 7
                    ,cols: [[ //表头
                        {field: 'id', title: titleMsg,width:150}
                        ,{field: 'tradeNo', title: '交易单号',width:180}
                        ,{field: 'userId', title: '用户Id', width:120}
                        ,{field: 'userName', title: '用户昵称', width:120,templet : function (d) {
                                var userName;
                                (d.userName == "" ? userName = "测试用户" : userName = d.userName);
                                return userName;
                            }}
                        ,{field: 'money', title: moneyInfo, width:120}
                        ,{field: 'desc', title: '备注', width:120}
                        ,{field: 'payType', title: '支付方式', width:120,templet : function (d) {
                                var payTypeMsg;
                                (d.payType == 1 ? payTypeMsg = "支付宝支付" : (d.payType == 2) ? payTypeMsg = "微信支付" : (d.payType == 3) ? payTypeMsg = "余额支付" : (d.payType == 4) ? payTypeMsg = "系统支付": payTypeMsg = "其他方式支付")
                                return payTypeMsg;
                            }}
                        ,{field: 'status', title: '交易状态', width:120,templet : function (d) {
                                var statusMsg;
                                (d.status == 0 ? statusMsg = "创建" : (d.status == 1) ? statusMsg = "支付完成" : (d.status == 2) ? statusMsg = "交易完成" :(d.status == -1) ? statusMsg = "交易关闭" : statusMsg = "关闭")
                                return statusMsg;
                            }}
                        ,{field: 'type', title: '类型', width:150, templet : function (d) {
                                var statusMsg;
                                (d.type == 10 ? statusMsg = "付款码付款": (d.type == 11) ?
                                    statusMsg = "付款码收款" : (d.type == 12) ? statusMsg = "二维码付款": (d.type == 13) ? statusMsg = "二维码收款": (d.type == 16) ? statusMsg = "后台手工提现"
                                        : (d.type == 18) ? statusMsg = "扫码手动充值" : (d.type == 19) ? statusMsg = "扫码手动提现" : statusMsg = "其他方式付款")
                                return statusMsg;
                            }}
                        ,{field: 'time',title:timeInfo,width:195,templet: function(d){
                                return UI.getLocalTime(d.time);
                            }}
                    ]]
                    ,done:function(res, curr, count){
                        checkRequst(res);
                        $(".user_table").hide();
                        $(".myBill").hide();
                        $("#myBillInfo").show();
                    }
                });
            }else if(14 == data.type || 15 == data.type){
                // 直播详情
                tableBillInfo = table.render({
                    elem: '#myBill_table_Info'
                    ,url:request("/console/consumeRecordInfo")+"&tradeNo="+data.tradeNo
                    ,id: 'myBill_table_Info'
                    ,page: {
                        layout: [ 'prev', 'page', 'next','limit', 'count', 'skip'] //自定义分页布局
                        ,groups: 3 //只显示 1 个连续页码
                        ,first: false //不显示首页
                        ,last: false //不显示尾页
                    }
                    ,curr: 0
                    ,limit:Common.limit
                    ,limits:Common.limits
                    ,groups: 7
                    ,cols: [[ //表头
                        {field: 'id', title: '礼物账单id', width:220}
                        ,{field: 'money', title: '价格', width:220}
                        ,{field: 'serviceCharge', title: '手续费', width:220}
                        ,{field: 'operationAmount', title: '实际收入', width:220}
                        ,{field: 'userId', title: '赠送人Id', width:220,templet:function (d) {
                                if(14 == data.type)
                                        return d.userId;
                                else
                                    return d.toUserId;
                            }}
                        ,{field: 'toUserId', title: '接收人Id', width:220,templet:function (d) {
                                if(14 == data.type)
                                    return d.toUserId;
                                else
                                    return d.userId;
                            }}
                        ,{field: 'time', title: '赠送时间', width:400,templet: function(d){
                                return UI.getLocalTime(d.time);
                            }}]]
                    ,done:function(res, curr, count){
                        checkRequst(res);
                        $(".user_table").hide();
                        $(".myBill").hide();
                        $("#myBillInfo").show();
                    }
                });

            }else {
                // if(7 == data.type || 8 == data.type || 9 == data.type)
                var titleName;
                data.type == 7 ? titleName = "转账用户昵称":titleName = "收取用户昵称"
                // 转账相关详情
                tableBillInfo = table.render({
                    elem: '#myBill_table_Info'
                    ,toolbar: '#toolbar'
                    ,url:request("/console/consumeRecordInfo")+"&tradeNo="+data.tradeNo
                    ,id: 'myBill_table_Info'
                    ,page: {
                        layout: [ 'prev', 'page', 'next','limit', 'count', 'skip'] //自定义分页布局
                        ,groups: 3 //只显示 1 个连续页码
                        ,first: false //不显示首页
                        ,last: false //不显示尾页
                    }
                    ,curr: 0
                    ,limit:Common.limit
                    ,limits:Common.limits
                    ,groups: 7
                    ,cols: [[ //表头
                        {field: 'id', title: '转账Id',width:200}
                        ,{field: 'userId', title: '转账用户Id',width:120,templet:function (d) {
                                if(7 == data.type)
                                    return d.userId;
                                else
                                    return d.toUserId;
                            }}
                        ,{field: 'userName', title: titleName, width:150,templet : function (d) {
                                if(Common.isNil(d.userName)){
                                    return;
                                }else{
                                    return d.userName;
                                }
                            }}
                        ,{field: 'toUserId', title: '收取用户Id', width:120,templet:function (d) {
                                if(7 == data.type)
                                    return d.toUserId;
                                else
                                    return d.userId;
                            }}
                        ,{field: 'money', title: '转账金额', width:120}
                        ,{field: 'remark', title: '转账说明', width:150,templet:function (d) {
                                var remarkInfo;
                                Common.isNil(d.transferRemark) ? remarkInfo = "" : remarkInfo = d.transferRemark;
                                return remarkInfo;
                            }}
                        ,{field: 'status', title: '转账状态', width:120,templet : function (d) {
                                var statusMsg;
                                (d.transferStatus == 1 ? statusMsg = "发出" : (d.transferStatus == 2) ? statusMsg = "已收款" : statusMsg = "已退款")
                                return statusMsg;
                            }}
                        ,{field: 'createTime', title: '转账时间', width:200,templet : function (d) {
                                return UI.getLocalTime(d.time);
                            }}

                    ]]
                    ,done:function(res, curr, count){
                        checkRequst(res);
                        $(".user_table").hide();
                        $(".myBill").hide();
                        $("#myBillInfo").show();
                    }
                });
            }

        }
    });

    // 红包账单明细
    table.on('tool(myBill_table_Info)', function(obj){
        var layEvent = obj.event,
            data = obj.data;
        if(layEvent === 'receiveWaterInfo'){
            console.log("红包流水"+JSON.stringify(data));
            var tableIns1 = table.render({
                elem: '#receiveWater_table'
                ,toolbar: '#toolbar'
                ,url:request("/console/receiveWater")+"&redId="+data.id
                ,id: 'receiveWater_table'
                ,page: {
                    layout: [ 'prev', 'page', 'next','limit', 'count', 'skip'] //自定义分页布局
                    ,groups: 3 //只显示 1 个连续页码
                    ,first: false //不显示首页
                    ,last: false //不显示尾页
                }
                ,curr: 0
                ,limit:10
                ,limits:[10,20,30,40,50,100,1000,10000]
                ,groups: 7
                ,cols: [[ //表头
                    {field: 'id', title: '领取流水Id'}
                    ,{field: 'redId', title: '红包Id'}
                    ,{field: 'sendName', title: '发送红包用户昵称'}
                    ,{field: 'userName', title: '领取红包用户昵称'}
                    ,{field: 'reply', title: '红包回复语'}
                    ,{field: 'money', title: '领取金额'}
                    ,{field: 'time', title: '领取时间',templet: function(d){
                            return UI.getLocalTime(d.time);
                        }}]]
                ,done:function(res, curr, count){
                    checkRequst(res);
                    $("#myBillInfo").hide();
                    $("#receiveWater").show();
                }
            });
        }
    });

    //搜索用户
    $(".search_user").on("click",function(){
        if($(".nickName").val().indexOf("*")!=-1){
            layer.alert("不支持*号搜索");
            return
        }
        // 关闭超出宽度的弹窗
        $(".layui-layer-content").remove();

        table.reload("user_list",{
            url:request('/console/userList'),
            page: {
                curr: 1 //重新从第 1 页开始
            },
            where: {
                onlinestate:$("#status").val(),// 在线状态
                keyWord : Common.getValueForElement(".nickName")  //搜索的关键字
            },
        })
        lock=1;
        exportOnline = $("#status").val();// 在线状态
        exportKeyword =  Common.getValueForElement(".nickName");  //搜索的关键字
        User.refreshMushDelete();
    });

    //搜索账单
    $(".search_bill").on("click",function(){
        // 关闭超出宽度的弹窗
        $(".layui-layer-content").remove();

        table.reload("myBill_table",{
            url:request("/console/userBill")+"&userId="+ Common.filterHtmlData(localStorage.getItem("currClickUser")),
            page: {
                curr: 1 //重新从第 1 页开始
            },
            where: {
                type:$("#operationType").val(),// 账单类型
            }
        })
        $("#type").val("");
    });


    //搜索邀请码
    $(".search_inviteCode").on("click",function(){

        table.reload("invite_code_table",{
            page: {
                curr: 1 //重新从第 1 页开始
            },
            where: {
                keyword : Common.getValueForElement(".invite_code_name"),  //搜索的关键字
                state: $(".inviteCodeStatus").val()
            }
        })

    });

    //生成注册型邀请码
    $(".btn_create_register_InviteCode").on("click",function(){

        layui.layer.open({
            title: '生成邀请码'
            ,skin: 'layui-ext-motif'
            ,type : 1
            ,offset: 'auto'
            ,area: '270px'
            ,btn: ['确定', '取消']
            ,content:'<div id="CreateInviteCode" style="margin:20px auto;">'
                +  '<div class="layui-inline" style="margin-left:20px;">'
                +      '<div class="layui-input-inline" style="min-width:200px;">'
                +         '<input id="inviteCodeNum" lay-verify="required|number" placeholder="生成的邀请码数量(1-3000) " autocomplete="off" class="layui-input" type="text">'
                +      '</div>'
                +  '</div>'
                +'</div>'
            ,yes: function(index, layero){
                //按钮【确定】 的回调
                var inviteCodeNum = Common.getValueForElement("#inviteCodeNum");
                var reg = /^[0-9]*[1-9][0-9]*$/;
                if(!reg.test(inviteCodeNum)){
                    layer.msg("生成数量必须是1-3000的数字",{icon: 2});
                    return;
                }else if(!(parseInt(inviteCodeNum)<=3000)){
                    layer.msg("生成数量必须是1-3000的数字",{icon: 2});
                    return;
                }

                Common.invoke({
                    url : request('/console/create/inviteCode'),
                    data : {
                        'nums':inviteCodeNum,
                        'userId':userId,
                        'type': 1
                    },
                    successMsg : "邀请码生成完成",
                    errorMsg : "邀请码生成失败,请稍后重试",
                    success : function(result) {
                        if (1 == result.resultCode){
                            //重载表格
                            table.reload("invite_code_table",{
                                page: {
                                    curr: 1 //重新从第 1 页开始
                                },
                                where: {
                                    keyWord : Common.getValueForElement(".invite_code_name"),  //搜索的关键字
                                    state: $(".inviteCodeStatus").val()
                                }
                            })

                        }
                    },
                    error : function(result) {
                    }
                });


                layui.layer.closeAll(); //关闭弹框
                layer.msg('开始生成邀请码');

            }
            ,btn2: function(index, layero){
                //按钮【取消】的回调

                //return false 开启该代码可禁止点击该按钮关闭
            },
            success: function(){

                layui.form.render();
            }
        })
    });


    $("#userDeviceInfo").click(function(){
        //设备信息
        var tableIns = table.render({
            elem: '#equipmentInfo_table'
            ,toolbar: '#toolbar'
            ,url: request("/console/userList")
            ,id: 'equipmentInfo_table'
            ,page: {
                layout: [ 'prev', 'page', 'next','limit', 'count', 'skip'] //自定义分页布局
                ,groups: 3 //只显示 1 个连续页码
                ,first: false //不显示首页
                ,last: false //不显示尾页
            }
            ,curr: 0
            ,limit:10
            ,limits:[10,20,30,40,50,100,1000,10000,100000]
            ,groups: 7
            ,cols: [[ //表头
                {field: 'userId', title: '所属用户Id',width:150},
                {field: 'telephone', title: '手机号码', width:135,templet:function(d){
                        if(0 == Config.getConfig().regeditPhoneOrName){
                            if(!Common.isNil(d.telephone))
                                return d.telephone;else return "";
                        }else{
                            return d.phone;
                        }
                    }},
                {field: 'nickname', title: '昵称',width:150},
                {field: 'userId', title: '安卓状态',width:130,templet(d){
                        var data = d.deviceInfo;
                        for(var key in data){
                            if ('android' == key){
                                return data[key].online==1?"在线":"离线"
                            }
                        }
                        return "-";
                    }},
                {field: 'demo', title: '安卓IP',width:150},
                {field: 'userId', title: '苹果状态',width:150,templet(d){
                        var data = d.deviceInfo;
                        for(var key in data){
                            if ('ios' == key){
                                return data[key].online==1?"在线":"离线"
                            }
                        }
                        return "-";
                    }},
                {field: 'demo', title: '苹果IP',width:150},
                {field: 'userId', title: 'PC状态',width:150,templet(d){
                        var data = d.deviceInfo;
                        for(var key in data){
                            if ('web' == key){
                                return data[key].online==1?"在线":"离线"
                            }
                        }
                        return "-";
                    }},
                {field: 'demo', title: 'PC IP',width:150},
            ]]
            ,done:function(res, curr, count){
                console.log(res);
                tableData = table.cache.invite_code_table;
                checkRequst(res);
                $(".user_table").hide();
                $(".user_btn_div").hide();
                $("#equipmentInfo").show();

                var pageIndex = tableIns.config.page.curr;//获取当前页码
                var resCount = res.count;// 获取table总条数
                currentCount = resCount;
                currentPageIndex = pageIndex;
            }
        });
    })

})

// 导出用户数据
function export_rand_user(){
    var requestUrl = request('/console/exportData');
    layui.layer.open({
        title: '导出用户数据'
        ,skin: 'layui-ext-motif'
        ,type : 1
        ,offset: 'auto'
        ,area: ['370px','160px']
        ,btn: ['导出', '取消']
        ,content:  '<form class="layui-form" style="padding: 0px 22px;" method="post" action='+requestUrl+'>'
            + '<div class="layui-form-item">'
            + 	'<div class="layui-inline">'
            + 		'<label>导出由系统自动生成的随机用户</label>'
            +     	'<div class="layui-input-inline" style="min-width:200px;margin-top:20px; display:none;">'
            +	      	    '<select name="userType">'
            +         	   '<option value="1">普通用户</option>'
            +	        	   '<option value="3" selected>随机用户(系统自动生成)</option>'
            +	      	    '</select>'
            +	        '</div>'
            +	     '</div>'
            + '</div>'
            +  '<button id="exportData_submit"  class="layui-btn" type="submit" lay-submit="" style="display:none">导出</button>'
            +'</from>'
        ,success: function(index, layero){
            layui.form.render();
        }
        ,yes: function(index, layero){
            //确定按钮的回调
            $("#exportData_submit").click();
            layui.layer.close(index); //关闭弹框
        }
        ,btn2: function(index, layero){
            //按钮【取消】的回调

            //return false 开启该代码可禁止点击该按钮关闭
        }

    });

};


//重新渲染表单
function renderTable(){
    layui.use('table', function(){
        var table = layui.table;//高版本建议把括号去掉，有的低版本，需要加()
        // table.reload("user_list");
        table.reload("user_list",{
            page: {
                curr: 1 //重新从第 1 页开始
            },
            where: {
                onlinestate:$("#status").val(),// 在线状态
                keyWord : Common.getValueForElement(".nickName")  //搜索的关键字
            }
        })
    });
}


var loginTime="";
var offlineTime="";

var User={
    balanceTotal:function(){
        layer.msg("正在计算，请稍等...",{icon:7});
        $(".loadingImg").show();

        //所有用户账户余额统计
        Common.invoke({
            url:request('/console/user/balance'),
            data:{},
            success:function(result){
                console.log(result);
                $(".loadingImg").hide();
                if(result.resultCode==1){
                    $(".allBalanceTotal").empty().text(Common.toDecimal2(result.data));
                }
            },
            error:function () {
                $(".loadingImg").hide();
            }
        });
    },
    //  新增用户
    addUser:function(){
        // 获取当前注册类型
        Common.invoke({
            url:request('/console/config'),
            data:{},
            success:function(result){
                if(result.resultCode==1){
                    regeditPhoneOrName = result.data.regeditPhoneOrName;
                    InviteCodeContest  = result.data.registerInviteCode;
                    console.log(result.data.registerInviteCode)
                    if(result.data.registerInviteCode == 0){
                        $("#InviteCodeIteam").hide();
                    }

                }
            }
        })
        /*for (area in countryInfo) {
            console.log(JSON.stringify(countryInfo[area][area]));
            $("#country").append("<option value='"+countryInfo[area][area]+"'>"+area+"</option>");
        }*/
        // renderForm(); //这个很重要
        $("#telephone").removeAttr("readOnly");
        $(".password").show();
        $("#userList").hide();
        $("#descriptionIteam").hide();
        $("#withdrawBurnReadTimeIteam").hide();
        $("#accountIteam").hide();
        $("#addUser").show();
        $("#userId").val(0);
        $("#userName").val("");
        $("#telephone").val("");
        $("#password").val("");
        $("#sex").val("");
        $("#isPublic").val("");

        // 重新渲染
        layui.form.render();
        $("#addUserTitle").empty();
        $("#addUserTitle").append("新增用户");
        $(".visitPathDiv").hide();
    },
    // 提交新增用户
    commit_addUser:function(){
        if($("#userName").val()==""){
            layer.msg("请输入昵称",{icon:2});
            return;
        }
        if($("#telephone").val()==""){
            // 第三方直接登录没有手机号
            layer.msg("请输入手机号码",{icon:2});
            return;
        }else{
            // regeditPhoneOrName 1:用户名，0：手机号
            if(0 == regeditPhoneOrName){
                var patrn = /^[0-9]*$/;
                if (patrn.exec($("#telephone").val()) == null || $("#telephone").val() == "") {
                    layer.msg("请使用手机号注册",{icon:2});
                    return;
                }
            }
        }

        var pwd = $("#password").val();
        if(0 == $("#userId").val() && Common.isNil(pwd)){
            layer.msg("请输入密码",{icon:2});
            return;
        }
        if (!pwd.match(array_alphabetic_regular)){
            layer.msg("密码错误，只能输入数字或字母",{icon:2});
            return;
        }


        if($("#sex").val()==""){
            layer.msg("请选择性别",{icon:2});
            return;
        }
        if($("#userId").val()!=0){
            if(Common.isNil($("#description").val())){
                layer.msg("请输入个性签名",{icon:2});
                return;
            }
        }

        var acc = $("#account").val();
        if(acc.match(pattern) || chains.test(acc)){
            layer.msg("通讯号只支持英文与数字，不支持特殊符号与中文！",{icon:2});
            return;
        }

        if("" == $("#isPublic").val() || null == $("#isPublic").val()){
            layer.msg("是否设置为公众号",{icon:2});
            return;
        }
        if (InviteCodeContest == 1){
            if($("#InviteCode").val()==""){
                layer.msg("请输入邀请码！",{icon:2});
                return;
            }
        }

        if (update_user == 1){
            if($("#account").val()==""){
                layer.msg("请输入通讯号！",{icon:2});
                return;
            }
            var withdrawBurnReadTime = $("#withdrawBurnReadTime").val();
            if(Common.isNil(withdrawBurnReadTime)){
                layer.msg("请输入撤回或即焚的保留时长！",{icon:2});
                return;
            }else if(!Common.isNil(withdrawBurnReadTime)){
                // 输入正整数且大于三十分钟
                if (!(/(^[0-9]\d*$)/.test(withdrawBurnReadTime))) {
                    layer.msg("请输入正确的撤回或即焚保留时长！",{icon:2});
                    return false;
                }else{
                    // 至少半个消息
                   if(0 != withdrawBurnReadTime && withdrawBurnReadTime < 10){
                       layer.msg("撤回或即焚保留时长最少为半个小时！",{icon:2});
                       return false;
                   }
                }
            }
        }

        $.ajax({
            url:request('/console/updateUser'),
            data:{
                userId:$("#userId").val(),
                nickname: Common.getValueForElement("#userName"),
                telephone: Common.getValueForElement("#telephone"),
                password: Common.getValueForElement("#password"),
                userType:$("#isPublic").val(),
                sex:$("#sex").val(),
                description: Common.getValueForElement("#description"),
                account: Common.getValueForElement("#account"),
                inviteCode: Common.getValueForElement("#InviteCode"),
                withdrawBurnReadTime: Common.getValueForElement("#withdrawBurnReadTime"),
                areaCode:$("#telephone").intlTelInput("getSelectedCountryData").dialCode
            },
            dataType:'json',
            async:false,
            success:function(result){
                checkRequst(result);
                if(result.resultCode==1){
                    if($("#userId").val()==0){
                        layer.msg("添加成功",{icon:1});
                        $("#userList").show();
                        $("#addUser").hide();
                        layui.table.reload("user_list",{
                            page: {
                                curr: 1 //重新从第 1 页开始
                            },
                            where: {

                            }
                        });
                        User.refreshMushDelete();
                    }else{
                        layer.msg("修改成功",{icon:1});
                        $("#userList").show();
                        $("#addUser").hide();
                        renderTable();
                    }

                }else{
                    layer.alert(result.resultMsg);
                }
            },
            error:function(result){
                if(result.resultCode==0){
                    layer.alert(result.resultMsg);
                }
            }
        })
    },
    // 修改用户
    updateUser:function(data,userId){
        $("#telephone").attr("readonly","readonly");
        $(".password").hide();
        $("#InviteCodeIteam").hide();
        $("#birthday").val("");
        $("#descriptionIteam").show();
        $("#withdrawBurnReadTimeIteam").show();
        $("#accountIteam").show();

        Common.invoke({
            url:request('/console/getUpdateUser'),
            data:{
                userId:userId
            },
            success:function(result){
                var userType = result.data.userType;

                console.log("type:"+result.data.userType);
                if(result.data!=null){
                    $("#userId").val(result.data.userId);
                    $("#userName").val(result.data.nickname);
                    $("#telephone").val(result.data.phone);
                    $('#isPublic').val(result.data.userType);
                    $("#sex").val(result.data.sex);
                    $("#description").val(result.data.description);
                    $("#account").val(result.data.account);
                    $("#withdrawBurnReadTime").val(result.data.withdrawBurnReadTime);
                }
                $("#addUserTitle").empty().text("修改用户");
                $("#userList").hide();
                $("#addUser").show();
                $(".visitPathDiv").hide();
                layui.form.render();
            }
        });

    },

    // 多选删除用户
    checkDeleteUsers:function(){
        layer.confirm('确定要删除？', {
            btn: ['确定','取消']
        }, function(){
            // 多选操作
            var checkStatus = layui.table.checkStatus('user_list'); //idTest 即为基础参数 id 对应的值

            for (var i = 0; i < checkStatus.data.length; i++){
                userIds.push(checkStatus.data[i].userId);
            }
            console.log(userIds);
            if(0 == checkStatus.data.length){
                layer.msg("请勾选要删除的行");
                return;
            }

            User.checkDeleteUsersImpl(userIds.join(","),checkStatus.data.length);
        });
    },

    checkDeleteUsersImpl:function(userId,checkLength){
        Common.invoke({
            url:request('/console/deleteUser'),
            data:{
                userId:userId
            },
            success:function(result){
                if(result.resultCode==1){
                    layer.msg("删除成功",{"icon":1});
                    User.refreshMushDelete();
                    Common.tableReload(currentCount,currentPageIndex,checkLength,"user_list");
                }
            },error:function () {
                userIds=[];
            }
        })
    },

    // 多选删除用户好友
    checkDeleteUsersFriends:function(){
        // 多选操作
        var checkStatus = layui.table.checkStatus('myFriends_table'); //idTest 即为基础参数 id 对应的值

        var userId;
        for (var i = 0; i < checkStatus.data.length; i++){
            toUserIds.push(checkStatus.data[i].toUserId);
            userId = checkStatus.data[i].userId;
        }
        console.log("userId: "+userId+"------"+toUserIds);
        if(0 == checkStatus.data.length){
            layer.msg("请勾选要删除的行");
            return;
        }
        User.checkDeleteUsersFriendsImpl(userId,toUserIds.join(","),checkStatus.data.length);
    },
    checkDeleteUsersFriendsImpl:function(userId,toUserId,checkLength){
        layer.confirm('确定删除指定好友',{icon:3, title:'提示消息',skin : "layui-ext-motif",yes:function () {
                Common.invoke({
                    url:request('/console/deleteFriends'),
                    data:{
                        userId:userId,
                        toUserIds:toUserId,
                        adminUserId:localStorage.getItem("account")
                    },
                    success:function(result){
                        if(result.resultCode==1){
                            layer.msg("删除成功",{"icon":1});
                            toUserIds = [];
                            // layui.table.reload("myFriends_table");
                            Common.tableReload(currentCount,currentPageIndex,checkLength,"myFriends_table");
                        }
                    }
                })
            },btn2:function () {
                toUserIds = [];
            },cancel:function () {
                toUserIds = [];
            }});
    },

    // 导出好友明细
    exportExcelFriends : function(){
        var requestUrl = request('/console/exportExcelByFriends');
        layui.layer.open({
            title: '导出好友数据'
            ,skin: 'layui-ext-motif'
            ,type : 1
            ,offset: 'auto'
            ,area: ['300px','180px']
            ,btn: ['导出', '取消']
            ,content:  '<form class="layui-form" method="post" action='+requestUrl+'>'
                + '<div class="layui-form-item">'
                + 	'<div class="layui-inline">'
                +	'<input type="hidden" name="userId" value='+userId+'>'
                + 		'<label class="layui-form-label" style="width: 90%;margin-top: 20px" >导出用户 “'+nickName+'” 的好友列表</label>'
                +	'</div>'
                + '</div>'
                +  '<button id="exportFriends_submit"  class="layui-btn" type="submit" lay-submit="" style="display:none">导出</button>'
                +'</from>'
            ,success: function(index, layero){
                layui.form.render();
            }
            ,yes: function(index, layero){
                $("#exportFriends_submit").click();
                layui.layer.close(index); //关闭弹框
            }
            ,btn2: function(index, layero){
                //按钮【取消】的回调

                //return false 开启该代码可禁止点击该按钮关闭
            }

        });

    },

    // 导出手机号明细
    exportExcelPhone : function(){
        /*  if(Common.isNil(exportStartTime)){
              layer.msg("请选择左侧的注册时间范围",{"icon":2});
              return;
          }*/
        exportStartTime = Common.isNil(exportStartTime) ? "" : exportStartTime;
        exportEndTime = Common.isNil(exportEndTime) ? "" : exportEndTime
        exportKeyword = Common.isNil(exportKeyword) ? "" : exportKeyword
        exportOnline = Common.isNil(exportOnline) ? "" : exportOnline
        var requestUrl = request('/console/exportExcelByPhone');
        console.log("request url export :"+requestUrl);
        layui.layer.open({
            title: '导出手机号数据'
            ,skin: 'layui-ext-motif'
            ,type : 1
            ,offset: 'auto'
            ,area: ['370px','160px']
            ,btn: ['导出', '取消']
            ,content:  '<form class="layui-form" style="padding:0px 22px;" method="post" action='+requestUrl+'>'
                + '<div class="layui-form-item">'
                + 	'<div class="layui-inline">'
                +	'<input type="hidden" name="startDate" value='+exportStartTime+'>'
                +	'<input type="hidden" name="endDate" value='+exportEndTime+'>'
                +	'<input type="hidden" name="keyWord" value='+exportKeyword+'>'
                +	'<input type="hidden" name="onlinestate" value='+exportOnline+'>'
                + 		'<label >导出当前筛选的手机号</label>'
                +	'</div>'
                + '</div>'
                +  '<button id="exportFriends_submit"  class="layui-btn" type="submit" lay-submit="" style="display:none">导出</button>'
                +'</from>'
            ,success: function(index, layero){
                layui.form.render();
            }
            ,yes: function(index, layero){
                $("#exportFriends_submit").click();
                layui.layer.close(index); //关闭弹框
            }
            ,btn2: function(index, layero){
                //按钮【取消】的回调
                //return false 开启该代码可禁止点击该按钮关闭
            }
        });
    },


    // 多选删除聊天记录
    toolbarUsersChatRecord:function(){
        // 多选操作
        var checkStatus = layui.table.checkStatus('friendsChatRecord_table'); //idTest 即为基础参数 id 对应的值
        console.log("新版："+checkStatus.data) //获取选中行的数据
        console.log("新版："+checkStatus.data.length) //获取选中行数量，可作为是否有选中行的条件
        console.log("新版："+checkStatus.isAll ) //表格是否全选
        for (var i = 0; i < checkStatus.data.length; i++){
            messageIds.push(checkStatus.data[i].messageId);
        }
        console.log(messageIds);
        if(0 == checkStatus.data.length){
            layer.msg("请勾选要删除的行");
            return;
        }
        User.toolbarUsersChatRecordImpl(messageIds.join(","),checkStatus.data.length);
    },
    toolbarUsersChatRecordImpl:function(messageId,checkLength){
        layer.confirm('确定删除指定聊天记录',{icon:3,skin : "layui-ext-motif", title:'提示消息',yes:function () {
                Common.invoke({
                    url:request('/console/delFriendsChatRecord'),
                    data:{
                        messageId :messageId
                    },
                    success:function(result){
                        if(result.resultCode==1){
                            layer.msg("删除成功",{"icon":1});
                            messageIds = [];
                            Common.tableReload(currentCount,currentPageIndex,checkLength,"friendsChatRecord_table");

                            // layui.table.reload("friendsChatRecord_table");
                        }
                    }
                })
            },btn2:function () {
                messageIds = [];
            },cancel:function () {
                messageIds = [];
            }});
    },

    // 重置密码
    ResetPassword:function(userId,areaCode,telephone,isOpenResetPwd){
        layui.layer.open({
            title:"重置密码",
            type: 1,
            btn:["确定","取消"],
            area: ['350px'],
            content: '<div id="mdifyPassword" class="layui-form" style="margin-top: 20px;">'
                +   '<div class="layui-form-item">'
                +      '<div class="layui-input-block" style="margin: 0 auto;">'
                +        '<input type="password" required  lay-verify="required" placeholder="新的密码" autocomplete="off" class="layui-input admin_passwd">'
                +      '</div>'
                +    '</div>'
                +   '<div class="layui-form-item">'
                +      '<div class="layui-input-block" style="margin: 0 auto;">'
                +        '<input type="password" required  lay-verify="required" placeholder="确认密码" autocomplete="off" class="layui-input admin_rePasswd">'
                +      '</div>'
                +    '</div>'
                +   (isOpenResetPwd == 1 ?  '<p>注：<span style="color: red">重置密码将重置端到端加密秘钥对，服务器上已加密的单聊聊天记录将无法解密！</span></p>' : '')
                +'</div>'

            ,yes: function(index, layero) {

                var newPasswd = Common.getValueForElement("#mdifyPassword .admin_passwd");
                var reNewPasswd = Common.getValueForElement("#mdifyPassword .admin_rePasswd");
                if(newPasswd!=reNewPasswd){
                    layui.layer.msg("两次密码输入不一致",{"icon":2});
                    return;
                } if(6>newPasswd.length){
                    layui.layer.msg("密码最少输入6位",{"icon":2});
                    return;
                }
                // 校验忘记密码调用的版本
                var isSupportParam = {};
                isSupportParam.areaCode = areaCode;
                isSupportParam.telephone = telephone;
                EncryptUtils.isSupportSecureChat(isSupportParam, function (result) {
                    isSupportParam.repassword = newPasswd;// 明文
                    console.log("result.isSupportSecureChat:"+result.isSupportSecureChat)
                    if (1 == result.isSupportSecureChat) {
                        // 新版
                        isSupportParam.salt = Common.getCurrentMilliSeconds();
                        EncryptUtils.resetPasswordV1(isSupportParam, function (resetPwd) {
                            layui.layer.close(index); //关闭弹框
                        })
                    } else {
                        var resetPwdParam = {};
                        resetPwdParam.areaCode = areaCode,
                            resetPwdParam.telephone = telephone,
                            /* resetPwdParam.randcode = obj.field.smsCode,*/
                            resetPwdParam.newPassword = newPasswd,

                            // 旧版
                           /* EncryptUtils.resetPassword(resetPwdParam, function (oldResetPwd) {
                                layui.layer.close(index); //关闭弹框
                            })*/

                        Common.invoke({
                            url : request('/console/updateUserPassword'),
                            data : {
                                "userId" : userId,
                                "password": $.md5(newPasswd)
                            },
                            successMsg : "重置密码成功",
                            errorMsg :  "重置密码失败，请稍后重试",
                            success : function(result) {
                                layui.layer.close(index); //关闭弹框
                                // location.replace("/pages/console/login.html");
                            },
                            error : function(result) {

                            }
                        });
                    }
                })
            }
        });
    },

    // 自动生成用户
    autoCreateUser:function(){
        /*弹窗*/
        layer.open({
            title:"随机生成用户",
            type: 1,
            btn: ['确定', '取消'],
            area: ['400px'],
            shade: [0.5, '#393D49'],
            skin: 'layui-ext-motif', //样式类名
            anim: 2,
            shadeClose: true, //开启遮罩关闭
            content: $("#autoCreateUser"),
            yes: function(index, layero){
                User.commit_autoCreateUser();
                layui.layer.close(index); //关闭弹框
            }
        });

    },
    // 提交自动生成用户
    commit_autoCreateUser:function(){
        if($("#userNum").val() <= 0){
            layer.msg("生成的用户数量至少为1个",{"icon":5});
            return;
        }
        Common.invoke({
            url:request('/console/autoCreateUser'),
            data:{
                userNum: Common.getValueForElement("#userNum")
            },
            success:function(result){
                if(result.resultCode==1){
                    layer.msg("随机生成用户成功",{"icon":1});
                    renderTable();
                }
            }
        })
    },
    // 导出用户
    exportUser:function(){
        $(".user_table").hide();
        $("#exportUser").show();
    },
    // 提交
    commit_exportUser:function(){
        Common.invoke({
            url:request('/console/exportData'),
            data:{
                userType: $("#userType").val()
            },
            success:function(result){
                if(result.resultCode==1){
                    layer.alert("导出成功");
                }
            }
        })
    },

    // 用户锁定解锁
    lockIng:function(userId,status){
        var confMsg,successMsg="";
        (status == -1 ? confMsg = '确定对该用户做封号处理？':confMsg = '确定解封该用户的账号？');
        (status == -1 ? successMsg = "封号成功":successMsg ="解封成功");
        layer.confirm(confMsg,{icon:3,skin : "layui-ext-motif", title:'提示信息'},function(index){

            Common.invoke({
                url : request('/console/changeStatus'),
                data : {
                    userId:userId,
                    status:status
                },
                successMsg : successMsg,
                errorMsg :  "加载数据失败，请稍后重试",
                success : function(result) {
                    layui.table.reload("user_list",{

                    })
                },
                errorCb : function(result) {
                }
            });
        })
    },

    // 加入移除黑名单
    joinMoveBlacklist:function(userId,toUserId,status){
        var confMsg,successMsg="";
        (status == 0 ? confMsg = '确定加入黑名单？':confMsg = '确定移除黑名单？');
        (status == 0 ? successMsg = "加入成功":successMsg ="移除成功");
        layer.confirm(confMsg,{icon:3,skin : "layui-ext-motif", title:'提示信息'},function(index){

            Common.invoke({
                url : request('/console/blacklist/operation'),
                data : {
                    userId:userId,
                    toUserId:toUserId,
                    type:status,
                    adminUserId:localStorage.getItem("account")
                },
                successMsg : successMsg,
                errorMsg :  "加载数据失败，请稍后重试",
                success : function(result) {
                    layui.table.reload("myFriends_table")
                },
                error : function(result) {
                }
            });
        })
    },

    // 发送消息
    sendUserMsg:function(toUserId){
        $(".user_btn_div").hide();
        $(".user_table").hide();
        $(".visitPathDiv").hide();
        $(".userListSearch").hide();
        $("#pushToUser").show();
        $("#push_userId").val(toUserId);// 发送者id
        $("#push_name").val('Customer service public account');// 发送者名称
    },
    // 发送
    commit_push:function(){
        if(null == $("#push_context").val() || "" == $("#push_context").val()){
            layer.alert("请输入要发送内容");
            return;
        }
        Common.invoke({
            url:request('/console/sendUserMsg'),
            data:{
                toUserId:$("#push_userId").val(),
                type:1,
                content:$("#push_context").val()
            },
            success:function(result){
                layer.alert("发送成功");
                $("#push_context").val("")
            }
        })
    },
    // 实人认证
    realNameCertified:function(userId){
        console.log("userId:",userId)
        var base64Data = "data:image/png;base64,";
        Common.invoke({
            url:request('/console/queryRealNameCertified'),
            data:{
                type:1,
                userId:userId
            },
            success:function(result){
                if(Common.isNil(result.data)){
                    layer.msg("该用户暂未实名认证", {icon: 2});
                    return;
                }
                var pattern = /(\d{4})(\d{2})(\d{2})/;
                if(!Common.isNil(result.data)){
                    $("#certifiedAccount").val(result.data.certifiedAccount);
                    $("#certifiedName").val(result.data.certifiedName);
                    $("#certifiedAddress").val(result.data.certifiedAddress);
                    $("#backImageUrl").attr("src",base64Data+result.data.backImageUrl);
                    $("#frontImageUrl").attr("src",base64Data+result.data.frontImageUrl);
                    $("#faceImageUrl").attr("src",base64Data+result.data.faceImageUrl);
                    var startDate = result.data.startDate.toString();
                    var endDate = result.data.endDate.toString();
                    if(1 == endDate){
                        endDate = "长期";
                    }else{
                        endDate = endDate.replace(pattern, '$1-$2-$3');
                    }
                    $("#startDate").val(startDate.replace(pattern, '$1-$2-$3')+" ~ "+endDate);
                }
                layui.layer.open({
                    title:"实人认证信息",
                    skin: 'layui-ext-motif',
                    type: 1,
                    btn:["确定","取消"],
                    area: ['750px','750px'],
                    content:$("#realPerson")
                    ,yes: function(index, layero){
                        layui.layer.close(index); //关闭弹框
                        $("#realPerson").css({"display":"none"});
                    }
                    ,cancel: function(){
                        $("#realPerson").css({"display":"none"});
                    }
                });
                $(".layui-layer-btn1").remove();// 隐藏取消按钮
            }
        })
    },

    button_back:function(){
        $("#userList").show();
        $(".user_table").show();
        $(".user_btn_div").show();
        $("#myBillInfo").hide();
        $(".billInfo").hide();
        $("#autoCreateUser").hide();
        $("#exportUser").hide();
        $("#addUser").hide();
        $("#myFriends").hide();
        $("#myInviteCode").hide();
        $(".myBill").hide();
        $("#pushToUser").hide();
        $("#equipmentInfo").hide();
        $(".visitPathDiv").show();
        $(".userListSearch").show();
        update_user = 0;
        User.refreshMushDelete();
    },
    // 好友聊天记录的返回按钮
    button_back_chatRecord:function(){
        $("#myFriends").show();
        $("#friendsChatRecord").hide();
    },

    // 账单明细的返回按钮
    button_back_userBillInfo:function(){
        $(".myBill").show();
        $("#myBillInfo").hide();
    },

    // 红包明细的返回按钮
    button_back_redBillInfo:function(){
        $("#myBillInfo").show();
        $("#receiveWater").hide();
    },

    //删除一对一邀请码
    deleteInviteCode:function (userId,inviteCodeId) {
        Common.invoke({
            url:request('/console/delInviteCode'),
            data:{
                userId:userId,
                inviteCodeId:inviteCodeId
            },
            success:function(result){
                layer.msg("删除成功", {icon: 1});
            }
        })
    },

    //禁用一对多邀请码
    prohibitInviteCode:function (userId,inviteCodeId,status) {
        Common.invoke({
            url:request('/console/prohibitInviteCode'),
            data:{
                userId:userId,
                inviteCodeId:inviteCodeId,
                status:status
            },
            success:function(result){
                if (status == 1){
                    layer.msg("开启成功", {icon: 1});
                    form.render();
                }else{
                    layer.msg("禁用成功", {icon: 1});
                    form.render();
                }

            }
        })
    }
    //分配权限
    ,addRole:function () {
        //用户编号
        var userId = localStorage.getItem("user_id");

        //角色权限
        var sele =  formSelects.value('select1', 'val');
        var roleAuth = "";
        for(var i = 0; i < sele.length; i++) {
            if (!sele[i].isIndeterminate){
                if (roleAuth === ""){
                    roleAuth = roleAuth+sele[i].id;
                }else{
                    roleAuth = roleAuth+","+sele[i].id;
                }
            }
        };
        alert(roleAuth);

        $.ajax({
            type:"POST",
            url:request("/console/updateRole"),
            data:{
                userId:userId,
                roleId:roleAuth

            },
            dataType:'json',
            async:false,
            success:function(result){
                if(result.resultCode==1){
                    layer.msg("添加成功！",{"icon":1});
                    setTimeout(function(){
                        window.location.href="/pages/console/userList.html";
                    }, 1500);
                }
            }
        })
    },
    /**
     * 打开操作
     */
    openOperation:function () {
        $(".layui-table-fixed-r").show()
    },

    /**
     * 关闭操作
     */
    closeOperation:function () {
        $(".layui-table-fixed-r").hide()
    },

    /**
     * 操作判断
     */
    operation:function (data) {
        if (operationFlag){
            User.closeOperation();
            if (data == 1){
                operationFlag = false;
            }
            $("#operation").html("打开操作");
        }else {
            User.openOperation();
            if (data == 1) {
                operationFlag = true;
            }
            $("#operation").html("关闭操作");
        }
    },
    /**
     * 展示地图
     */
    showMap:function (lng,lat) {
        var img = mapApi + lng + "," + lat;
        var html = "";
        html += '<div style="padding: 2%;">' +
            '<label class="layui-text layui-word-aux">经度：' + lng + ", 维度：" + lat + '</label>' +
            '</div>';
        html += '<img style="margin: 10px 0px 0px 10px;border: 1px solid red;" src=' + img + '>';
        //弹出层
        layer.open({
            type: 1,
            title: "预览地址",
            offset: ['50px'],
            area: ['700px', '700px'],
            shadeClose: true,
            shade: false,
            maxmin: true, //开启最大化最小化按钮
            content: html
        });
    },
    /**
     * 刷新多选删除
     */
    refreshMushDelete:function () {
        chooseDeleteCount = 0;
        $(".mustDelete").hide();
        userIds = [];
    }

}
let country='';
layui.form.on('select(country)',function(data){

    $("#province").empty();
    $("#province").append("<option value=''>请选择省份</option>");
    $("#city").empty();
    $("#city").append("<option value=''>请选择城市</option>");
    country=data.value;
    for(s in provinceInfo[data.value]){
        $("#province").append("<option value='"+s+"'>"+provinceInfo[data.value][s]+"</option>");
        renderForm();
    }
})

layui.form.on('select(province)',function(data){
    $("#city").empty();
    $("#city").append("<option value=''>请选择城市</option>");
    for(c in cityInfo[data.value]){
        $("#city").append("<option value='"+c+"'>"+cityInfo[data.value][c]+"</option>");
        renderForm();
    }
})

//重新渲染表单
function renderForm(){
    //use加载所需模块
    layui.use('form', function(){
        var form = layui.form;//高版本建议把括号去掉，有的低版本，需要加()
        form.render();
    });
}
$(function () {
    //调用父级页面的Js函数
    window.parent.getJointVisitPath();
})