layui.use(['form','layer','laydate','table','laytpl'],function(){
    var form = layui.form,
	    ...
	    var tableIns = table.render({
	      ...
	      ,groups: 7
	      ,cols: [[ //表头
	           {field: 'userId', title: '用户Id',width:'13%'}
	          ,{field: 'mail', title: '绑定邮箱账号',width:'10%'}
	          ,{field: 'telephone', title: '手机号',  width:'13%'}
	          ,{field: 'realName', title: '真实姓名', width:'10%'}
	          ,{field: 'verifyTime', title: '审核认证时间', width:'10%',templet: function(d){
	          		return (d.verifyTime!=undefined?UI.getLocalTime(d.verifyTime):"");
	          }}
	          ,{field: 'endTime', title: '到期时间', width:'13%'}
	          ,{field: 'createTime',title:'申请时间', width:'13%',templet: function(d){
	          		return UI.getLocalTime(d.createTime);
	          }}
	          ,{fixed: 'right',title:"操作", width:'10%', align:'left', toolbar: '#openDeveloperListBar'}
	        ]]
			,done:function(res, curr, count){
				checkRequst(res);
				//权限判断
				var arr=['developer-applicationList','developer-chatRecord','developer-member','developer-randUser','developer-modifyConf','developer-msgCount','developer-detail','developer-delete'];
				manage.authButton(arr);
				//获取零时保留的值
				var last_value = $("#openDeveloper_limlt").val();
				//获取当前每页大小
				var recodeLimit =  tableIns.config.limit;
				//设置零时保留的值
				$("#openDeveloper_limlt").val(recodeLimit);
				//判断是否改变了每页大小数
...
var Developer={
		...
		Common.invoke({
			...
			success:function(result){
				...
				if(result.data.status==1){
					...
					$("#undisable").hide();
					$("#disable").show();
				}else if(result.data.status==-1){// 禁用
					$("#disable").hide();
					$("#approvedDeveloper").hide();
					$("#reasonFailure").hide();
					$("#undisable").show();
				}else if(result.data.status==0||result.data.status==2){
					$("#approvedDeveloper").show();
					$("#reasonFailure").show();
					$("#undisable").hide();
					$("#disable").hide();
				}else {
				...
				$(".visitPathDiv").hide();
				$(".searchOpenDeveloper").hide();
				$("#userId").empty();
				$("#userId").append(Common.filterHtmlData(result.data.userId));
				$("#realName").empty();
				$("#realName").append(Common.filterHtmlData(result.data.realName));
				$("#telephone").empty();
				$("#telephone").append(Common.filterHtmlData(result.data.telephone));
				$("#mail").empty();
				$("#mail").append(Common.filterHtmlData(result.data.mail));
				$("#idCard").empty();
				$("#idCard").append(Common.filterHtmlData(result.data.idCard));
				$("#address").empty();
				$("#address").append(Common.filterHtmlData(result.data.address));
				$("#companyName").empty();
				$("#companyName").append(Common.filterHtmlData(result.data.companyName));
				$("#businessLicense").empty();
				$("#businessLicense").append(Common.filterHtmlData(result.data.businessLicense));
				$("#createTime").empty();
	...
	// 删除开发者
	deleteDeveloper:function(obj){
		var layEvent = obj.event,
			data = obj.data;
		layer.confirm('确定删除该开发者？',{icon:3, title:'提示信息',skin : "layui-ext-motif"},function(index){
			Common.invoke({
		...
		Common.invoke({
			...
			success:function(result){
				...
				for(var i=0;i<result.data.length;i++){
					...
					html+="<tr><td>"+Common.filterHtmlData(result.data[i].userId)+"</td><td>"+Common.filterHtmlData(result.data[i].mail)+"</td><td>"
					+Common.filterHtmlData(result.data[i].telephone)+"</td><td>"+Common.filterHtmlData(result.data[i].realName)+"</td><td>"
					...
					+"</td><td><a class='layui-btn table_default_btn layui-btn-xs' onclick='Developer.developerDetail(\""
					+result.data[i].userId+"\")'>详情</a><a class='layui-btn table_default_btn layui-btn-xs' onclick='Developer.deleteDeveloper(\""
