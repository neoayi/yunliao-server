...
layui.use(['form','layer','laydate','table','laytpl'],function(){
    ...
    var tableIns = table.render({
      ...
      ,cols: [[ //表头
        ...
        ,done:function(res){
            ...
            //权限判断
            var arr=['cus-delete','cus-randUser','cus-promotionUrl','cus-locking','cus-resource','cus-add','supperAdmin'];
            manage.authButton(arr);


            //获取零时保留的值
            var last_value = $("#customer_limlt").val();
            //获取当前每页大小
            var recodeLimit =  tableIns.config.limit;
            //设置零时保留的值
            $("#customer_limlt").val(recodeLimit);
            //判断是否改变了每页大小数
    ...
    table.on('tool(admin_list)', function(obj){
        ...
        if(layEvent === 'delete'){ //删除
         	 ...
         	 layer.confirm('确定删除该客服？',{icon:3, title:'提示信息',skin : "layui-ext-motif"},function(index){
            ...
            layui.layer.open({
                ...
                btn:["确定","取消"],
                area: ['310px'],
                content: '<div id="changePassword" style="padding: 15px 40px 0px 40px;">'
            ...
            layui.layer.open({
                ...
                success : function(){  //弹窗打开成功后的回调
                    $(".updatePromotionUrl").val( Common.filterHtmlData(data.promotionUrl) );
                }
                ,yes: function(index, layero){ //确定按钮的回调
            ...
            localStorage.setItem("start_page","/pages/console/customer.html");
    ...
    function updateAdmin(adminId,userId,status,infoStr,role,callback) {
	    ...
	    Common.invoke({
		      ...
		      data : {
                ...
                "userId": userId,
                "status": status,
                "promotionUrl": Common.getValueForElement(".updatePromotionUrl"),
                "role":role
