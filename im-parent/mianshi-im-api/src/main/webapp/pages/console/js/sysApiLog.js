...
layui.use(['form','layer','laydate','table','laytpl'],function(){
    ...
    var tableIns = table.render({
      ...
      ,cols: [[ //表头
           ...
           ,{field: 'totalTime', title: '耗时(毫秒)', width:'8%'}
           ,{field: 'time',title:'调用时间', width:'11%',templet: function(d){
          		// return Common.formatDate(d.time,"yyyy-MM-dd hh:mm:ss");
          		return UI.getLocalTime(d.time);
          }}
          ,{field: 'fullUri', title: '完整url', width:'20%'}
          ,{field: 'userId', title: '调用者Id',width:'6%'}
          ,{field: 'clientIp', title: '调用端ip', width:'11%'}
          ,{field: 'userAgent', title: '调用端信息', width:'11%'}
          ,{field: 'stackTrace', title: '错误信息', width:'11%'}
  ...
  table.on('tool(sysApiLogs_table)', function(obj){
    ...
    $(".search_sysApiLog").on("click",function(){
        if($(".sysApiLog_name").val().indexOf("*")!=-1){
            layer.alert("不支持*号搜索")
            return
        }
        // 关闭超出宽度的弹窗
        ...
        table.reload("sysApiLogs_table",{
            ...
            where: {
                keyWorld : Common.getValueForElement(".sysApiLog_name")  //搜索的关键字
            }
        })
    });
    ...
    $(".deleteSevenDaysAgoLogs").on("click",function(){
        ...
        layer.confirm('确定删除七天前的接口日志？',{icon:3,skin : "layui-ext-motif", title:'提示信息'},function(index){
