...
layui.use(['bodyTab','form','element','layer','jquery'],function(){
	...
	$(".adminName").append(localStorage.getItem("nickname"));
...
<!--                    <button id="confirmButton" class="layui-btn layui-btn-normal" style="margin-top: 20px;">确定已扫码</button>-->
			...
			success: function(layero, index){
				// 发起Ajax请求，获取二维码URL
				$.ajax({
					url: '/console/getQrcodeV2?userId='+localStorage.getItem("userId"), // 替换为你的后端接口
					type: 'GET',
    ...
    $("#changeMyPasswd").on("click",function(){
        ...
        layui.layer.open({
            ...
            btn:["确定","取消"],
            area: ['540px'],
            content: '<div id="mdifyPassword" class="layui-form" style="margin:20px 76px 10px 40px;">'
        	+   '<div class="layui-form-item">'
			+      '<label class="layui-form-label">旧密码</label>'
            +      '<div class="layui-input-block">'
            +        '<input type="password" required  lay-verify="required" placeholder="请输入" autocomplete="off" class="layui-input admin_oldPasswd">'
            +      '</div>'
            +    '</div>'
            +   '<div class="layui-form-item">'
			+      '<label class="layui-form-label">新的密码</label>'
            +      '<div class="layui-input-block">'
            +        '<input type="password" required  lay-verify="required" placeholder="请输入" autocomplete="off" class="layui-input admin_passwd">'
            +      '</div>'
            +    '</div>'
            +   '<div class="layui-form-item">'
			+      '<label class="layui-form-label">确认密码</label>'
            +      '<div class="layui-input-block">'
            +        '<input type="password" required  lay-verify="required" placeholder="请输入" autocomplete="off" class="layui-input admin_rePasswd">'
            +      '</div>'
            +    '</div>'
            +'</div>'
            ...
            ,yes: function(index, layero){ //确定按钮的回调
            	...
            	var oldPasswd = Common.getValueForElement("#mdifyPassword .admin_oldPasswd");
	...
	form.on('select(switchTest)', function(data){
		if (Common.isNil(data.value)){return;}
		$("#iframe").attr('src',data.value);
	});
})
...
/*设置顶部导航*/
function setTopMenu(){
	var innerWidth = getScreenWidth();
	if (innerWidth < 1920){
		$(".top_menu_main").css("width",innerWidth-310);
	}
}
/*屏幕窗口变化事件*/
...
$(function () {
	...
	$(".layui-nav-item_").hover(function(){
		$(".layui-nav-bar").hide();
	});
