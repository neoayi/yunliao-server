layui.use(['form','layer','laydate','table','laytpl'],function() {
    ...
    var tableIns = table.render({
        ...
        ,groups: 7
        ,cols: [[ //表头
            {field: 'userId', title: '充值用户Id',width:'10%'}
            ,{field: 'nickName', title: '充值用户昵称',width:'10%'}
            ,{field: 'payName', title: '付款方姓名',width:'10%'}
            ,{field: 'rechargePresentedScale', title: '赠送比例',width:'10%'}
            ,{field: 'money', title: '充值金额', width:'10%'}
            ,{field: 'currentMoney', title: '当前余额', width:'10%'}
            ,{field: 'type', title: '充值方式', width:'10%',templet : function (d) {
                    return d.type==1?"微信充值":d.type==2?"支付宝充值":"银行卡充值"
                }}
            ,{field: 'orderNo', title: '订单号', width:'10%'}
            // ,{field: 'serviceCharge', title: '手续费', width:120}
            // ,{field: 'actualMoney', title: '实际金额', width:120}
            ...
            ,{fixed: 'right',title:"操作", align:'left', width:'15%', toolbar: '#checkRechargeBar'}
        ]]
        ,done:function(res, curr, count){
            checkRequst(res);

            //权限判断
            var arr=['scanGathering-pass','scanGathering-ignore','scanGathering-delete'];
...
var Manual_Rech={
    ...
    approveRecharge:function (id) {
        layer.confirm('确定通过审核条充值申请？',{icon:3, title:'提示信息',skin : "layui-ext-motif"},function(index){
            Common.invoke({
    ...
    ignoreRecharge:function (id) {
        layer.confirm('确定忽略该条充值申请？',{icon:3, title:'提示信息',skin : "layui-ext-motif"},function(index){
            Common.invoke({
                url:request("/manualAdmin/checkRecharge"),
                data:{
                    id:id,
    ...
    deleteRecharge:function (id) {
        layer.confirm('确定删除该条充值申请？',{icon:3, title:'提示信息',skin : "layui-ext-motif"},function(index){
            Common.invoke({
