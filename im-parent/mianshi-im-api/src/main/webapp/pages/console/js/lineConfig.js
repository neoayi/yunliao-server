...
layui.use(['jquery', 'form', 'layer', 'laydate', 'table', 'laytpl'], function () {
    ...
    var tableInUser = table.render({
        elem: '#config_list'
        , toolbar: '#toolbarRole'
        , url: request("/console/listCollectionAccount")
        , id: 'config_list'
        , page: {
        ...
        , groups: 7
        , cols: [[ //表头
            {field: 'accountName', title: '收款人名字', sort: 'true', width: 300},
            {field: 'account', title: '收款人账号', sort: 'true', width: 300}
    ...
    table.on('tool(config_list)', function (obj) {
        ...
        if (layEvent === 'update') {
            //修改角色
            Manage.showRoleDesc(obj.data.roleId);
            ...
            layer.confirm('确认要删除吗？', {
                ...
                $.ajax({
                    type: "POST",
                    url: request("/console/deleteCollectionAccount"),
                    data: {
                        id: obj.data.id
                    },
                    dataType: 'json',
...
var Manage = {
    ...
    , submitAddRole: function () {
        ...
        if (roleName === "") {
            layer.msg('收款账户不能为空！', {icon: 5});
            return;
        }
        var roleDesc = $("#roleDesc").val();
        // if (roleDesc === "") {
        //     layer.msg('线路地址不能为空！', {icon: 5});
        //     return;
        // }
        ...
        $.ajax({
            type: "POST",
            url: request("/console/saveCollectionAccount"),
            data: {
            ...
            success: function (result) {
                if (result.resultCode == 1) {
                    ...
                    setTimeout(function () {
                        window.location.href = "/pages/console/lineConfig.html";
                    }, 1500);
                } else {
