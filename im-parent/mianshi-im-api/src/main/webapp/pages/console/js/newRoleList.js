...
layui.use(['form','layer','laydate','table','laytpl'],function(){
    ...
    var tableIns = table.render({
      elem: '#admin_list'
      ,toolbar: '#toolbar'
      ,url:request("/console/queryUserByRole")+"&type="+localStorage.getItem("addNewRoleUser_role")
      ,id: 'admin_list'
      ,page: true
      ,curr: 0
      ,limit:Common.limit
    ...
    table.on('tool(admin_list)', function(obj){
        ...
        if(layEvent === 'delete'){ //删除
         	...
         	layer.confirm('确定删除该用户？',{icon:3, title:'提示信息',skin : "layui-ext-motif"},function(index){
         			
         			Common.invoke({
					      url : request('/console/deleteUserByRole'),
					      data : {
                            "id" :data.id
			          	},
					    successMsg : "删除成功",
            ...
            layui.layer.open({
                ...
                content: '<div id="changePassword" class="layui-form" style="margin:20px 40px 10px 40px;;">'
                         ...
                         +        '<input type="password" required  lay-verify="required" placeholder="输入新的密码" autocomplete="off" class="layui-input admin_passwd">'
                         +      '</div>'
                         +    '</div>'
                         +   '<div class="layui-form-item">'
                         +      '<div class="layui-input-block" style="margin: 0 auto;">'
            ...
            updateAdmin(localStorage.getItem("account"),data.userId,-1,"禁用该用户",data.role, function(){
                        //layui.layer.close(index); //关闭弹框
                   		table.reload("admin_list",{
  			                page: {
  			                    curr: 1 //重新从第 1 页开始
            ...
            data.state = 1;
            updateAdmin(localStorage.getItem("account"),data.userId,1,"解禁该用户",data.role, function(){
                        //layui.layer.close(index); //关闭弹框
                   		table.reload("admin_list",{
			                page: {
			                    curr: 1 //重新从第 1 页开始
            ...
            localStorage.setItem("start_page","/pages/console/newRoleList.html");
    ...
    $(".btn_addAdmin").on("click",function(){
			...
			layui.layer.open({
                ...
                yes: function(index, layero){
      					...
      					Common.invoke({
    					      url : request('/console/addRoleUser'),
    					      data : {
                                phone:account,
                                rId:localStorage.getItem("addNewRoleUser_role"),
                                roleId:localStorage.getItem("addNewRoleUser_roleId")
    					      },
    					      successMsg : "创建成功",
    					      errorMsg :  "创建失败，请稍后重试",
    					      success : function(result) {
