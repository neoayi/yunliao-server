    ...
    var form = layui.form,
	    ...
	    var tableIns = table.render({
	      ...
	      ,cols: [[ //表头
	          ...
	          ,{field: 'appId', title: 'appId',width:'15%'}
	          ,{field: 'operateUser', title: '操作用户Id', width:'15%'}
	          ,{field: 'status', title: '审核结果', width:'15%',templet:function(d){
	          		return (d.status==1?"审核通过":d.status==2?"审核失败":d.status==0?"审核中":"禁用");
	          }}
	          ,{field: 'reason', title: '审核回馈', width:'15%'}
	          ,{field: 'createTime',title:'操作时间', width:'15%',templet: function(d){
	          		return UI.getLocalTime(d.createTime);
	          }}
	          ,{fixed: 'right',title:"操作", width:'7%', align:'left', toolbar: '#openCheckLogListBar'}
	        ]]
			,done:function(res, curr, count){
				checkRequst(res);
				//权限判断
				var arr=['checkLog-detail','checkLog-delete'];
	    ...
	    table.on('tool(openCheckLog_table)', function(obj){
	        ...
	        if(layEvent === 'del'){ //删除日志
                ...
                layer.confirm('确定删除日志',{icon:3, title:'提示消息',skin : "layui-ext-motif",yes:function (index) {
