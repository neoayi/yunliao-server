$(function () {
    ...
    if (operate == 1){
        $("#chat-card-header").append("添加部门");
        $("#chat-from-yg-update").hide();
        $("#chat-from-bm-update").hide();
        $("#chat-from-bm-addEmp").hide();
    }else if(operate == 2){
        $("#chat-card-header").append("修改部门");
        $("#chat-from-yg-update").hide();
        $("#chat-from-bm-add").hide();
        $("#chat-from-bm-addEmp").hide();
        loadDepMsg()
    }else if(operate == 3){
        $("#chat-card-header").append("添加员工");
        $("#chat-from-bm-add").hide();
        $("#chat-from-bm-update").hide();
        $("#chat-from-yg-update").hide();
    }else{
        $("#chat-card-header").append("修改员工信息");
        $("#chat-from-bm-add").hide();
        $("#chat-from-bm-update").hide();
        $("#chat-from-bm-addEmp").hide();
        load();
    ...
    function loadDepMsg() {
        ...
        $.ajax({
            ...
            success:function(result){
                ...
                $("#newDepartName").val( Common.filterHtmlData(localStorage.getItem("chat_dep_name")) );
                if(result.data!=null){
                    /*渲染下拉框数据*/
                    for(var i=0;i<result.data.length;i++){
                        if (localStorage.getItem("chat_dep_id") != result.data[i].id){
            ...
            success:function(result){
                if(result.data!=null){
                    ...
                    $.ajax({
                        ...
                        data:{
                            id:localStorage.getItem("chat_dep_id")
                        },
                        dataType:'json',
                        async:false,
                        success:function(result){
    ...
    /*添加部门*/
    $("#chat-submit-add").click(function () {
        ...
        if (localStorage.getItem("chat_dep_id") == 0){
        ...
        $.ajax({
            ...
            data:{
                companyId : Common.filterHtmlData(localStorage.getItem("company_Id")),
                parentId : Common.filterHtmlData(localStorage.getItem("chat_dep_id")),
    ...
    /*修改部门信息*/
    $("#chat-submit-upadte").click(function () {
        ...
        $.ajax({
            ...
            data:{
                //当前部门编号
                departmentId: Common.filterHtmlData(localStorage.getItem("chat_dep_id")),
                //修改的部门名称
                newDpartmentName: Common.filterHtmlData($("#newDepartName").val()),
                //移动到那个部门
                newDepId: Common.filterHtmlData($("#moveDep").val()),
                //旧部门名称
                oldDpartmentName: Common.filterHtmlData(localStorage.getItem("chat_dep_name"))
            },
            dataType:'json',
            async:false,
            success:function(result){
   ...
   /* 部门添加员工*/
    $("#chat-submit-addEmp").click(function () {
