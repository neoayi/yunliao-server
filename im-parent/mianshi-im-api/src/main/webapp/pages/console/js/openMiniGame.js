layui.use(['form','layer','laydate','table','laytpl'],function(){
    var form = layui.form,
	    ...
	    var tableIns = table.render({
	      ...
	      ,url:request("/console/openAppList")+"&type=3"
		  ,toolbar: '#toolbar'
	      ,id: 'openWebApp_table'
	      ,page: true
	      ,curr: 0
	      ...
	      ,groups: 7
	      ,cols: [[ //表头
	           {field: 'accountId', title: '申请用户Id',width:'15%'}
	          ,{field: 'appName', title: '应用名称',width:'15%'}
	          ,{field: 'appIntroduction', title: '应用简介', width:'15%'}
	          ,{field: 'appUrl', title: '应用官网', width:'15%'}
	          ,{field: 'createTime',title:'申请时间', width:'15%',templet: function(d){
	          		return UI.getLocalTime(d.createTime);
	          }}
	          ,{fixed: 'right',title:"操作", align:'left', toolbar: '#openWebAppListBar'}
	        ]]
			,done:function(res, curr, count){
				checkRequst(res);
				//权限判断
				var arr=['webUse-applicationList','webUse-chatRecord','webUse-member','webUse-randUser','webUse-modifyConf','webUse-msgCount','webUse-detail','webUse-delete'];
...
var WebApp={
	...
	deleteWebApp:function(id,accountId,obj){
		layer.confirm('确定删除该应用？',{icon:3, title:'提示信息',skin : "layui-ext-motif"},function(index){
			Common.invoke({
			...
			success:function(result){
				...
				if(result.resultCode==1){
					...
					for(var i=0;i<result.data.length;i++){
						...
						+UI.getLocalTime(result.data[i].createTime)+"</td><td><a class='layui-btn table_disabled_btn layui-btn-xs' onclick='WebApp.appDetail(\""
						+result.data[i].id+"\")'>详情</a><a class='layui-btn table_default_btn layui-btn-xs' onclick='WebApp.deleteWebApp(\""
