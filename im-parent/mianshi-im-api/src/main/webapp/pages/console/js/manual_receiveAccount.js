layui.use(['form','layer','laydate','table','laytpl'],function() {
    ...
    var tableIns = table.render({
        ...
        ,groups: 7
        ,cols: [[ //表头
            {field: 'id', title: '收款账户Id',width:'12%'}
            ,{field: 'type', title: '类型',width:'12%',templet : function (d) {
                    return d.type==1?"微信账户":d.type==2?"支付宝账户":"银行卡账户"
                }}
            ,{field: 'url', title: '二维码地址', width:'12%'}
            ,{field: 'name', title: '账户名称', width:'12%'}
            ,{field: 'payNo', title: '微信或支付宝账号', width:'12%'}
            ,{field: 'bankCard', title: '银行卡号', width:'12%'}
            ,{field: 'bankName', title: '开户银行名称', width:'12%'}
            ,{fixed: 'right',title:"操作", align:'left', toolbar: '#receiveAccountBar'}
        ]]
        ,done:function(res, curr, count){
            checkRequst(res);

            //权限判断
            var arr=['gathering-delete','gathering-update','gathering-add'];
    ...
    form.on("select(account_type)",function(){
        ManualPay.changeSelect();
    })
...
var ManualPay = {
    ...
    deleteReceiveAccount:function(id){
        layer.confirm('确定删除该收款账户？',{icon:3, title:'提示信息',skin : "layui-ext-motif"},function(index){
            Common.invoke({
