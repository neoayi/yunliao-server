...
layui.use(['form', 'layer', 'laydate', 'table', 'laytpl'], function () {
    ...
    var tableIns = table.render({
        ...
        , cols: [[ //表头
            ...
            , {
                field: 'userName', title: '付款用户昵称',  width: '10%', templet: function (d) {
                    var userName;
                    (d.userName == "" ? userName = "测试用户" : userName = d.userName);
                    return userName;
                }
            }
            , {field: 'money', title: '付款金额',  width: '10%', totalRow: true}
        ...
        , done: function (res, curr, count) {
            checkRequst(res);
            //获取零时保留的值
            var last_value = $("#paymentCode_limlt").val();
            //获取当前每页大小
            var recodeLimit = tableIns.config.limit;
            //设置零时保留的值
            $("#paymentCode_limlt").val(recodeLimit)
    ...
    $(".search_paymentCode").on("click", function () {
        ...
        table.reload("paymentCode_table", {
            url: request("/console/paymentCodeList"),
            where: {
                userId: Common.getValueForElement(".paymentCode_keyword"), //搜索的关键字
            },
            page: {
                curr: 1 //重新从第 1 页开始
            }
