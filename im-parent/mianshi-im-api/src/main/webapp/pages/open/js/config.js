...
function createCommApiSecret(obj){
		...
		if(!myFn.isNil(obj.userId)&&!myFn.isNil(obj.access_token)){
			key = localStorage.getItem("apiKey")+obj.time+obj.userId+obj.access_token;
		}else{
			key = localStorage.getItem("apiKey")+obj.time+localStorage.getItem("userId")+localStorage.getItem("access_token");
		}
