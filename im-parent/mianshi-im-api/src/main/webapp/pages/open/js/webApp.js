...
var Web={
	...
	upBt:function(num){
		if(num==1){
			$("#step1").show();
			$("#step2").hide();
			$("#title_solid_one").removeClass("img_color");
		}else if(num==2){
			$("#interfaceInfo").show();
			$("#payCallBack").hide();
		}
	...
	lookInfo:function(id){
		...
		myFn.invoke({
			...
			success:function(result){
				...
				$("#isAuthShare").empty();
				$("#isAuthShare").append(result.data.isAuthShare==2?"申请中":result.data.isAuthShare==1?"已获得":result.data.isAuthShare==-1?"审核失败":"未获得");
				$("#isAuthLogin").empty();
				$("#isAuthLogin").append(result.data.isAuthLogin==2?"申请中":result.data.isAuthLogin==1?"已获得":result.data.isAuthLogin==-1?"审核失败":"未获得");
				$("#isAuthPay").empty();
				$("#isAuthPay").append(result.data.isAuthPay==2?"申请中":result.data.isAuthPay==1?"已获得":result.data.isAuthPay==-1?"审核失败":"未获得");
				$("#isGroupHelper").empty();
				$("#isGroupHelper").append(result.data.isGroupHelper==2?"申请中":result.data.isGroupHelper==1?"已获得":result.data.isGroupHelper==-1?"审核失败":"未获得");
				$("#isAuthShareOperate").empty();
				$("#isAuthShareOperate").append(result.data.isAuthShare==0||result.data.isAuthShare==-1?"<a onclick='Web.applicationShare(\""+result.data.id+"\")' style='color:#3292ff;cursor:pointer;'>申请开通</a>":"<a onclick='' style='cursor:pointer;'>--<a>");
				$("#isAuthLoginOperate").empty();
				$("#isAuthLoginOperate").append(result.data.isAuthLogin==0||result.data.isAuthLogin==-1?"<a onclick='Web.applicationLogin(\""+result.data.id+"\")' style='color:#3292ff;cursor:pointer;'>申请开通</a>":"<a onclick='' style='cursor:pointer;'>--</a>");
				$("#isAuthPayOperate").empty();
				$("#isAuthPayOperate").append(result.data.isAuthPay==0||result.data.isAuthPay==-1?"<a onclick='Web.applicationPay(\""+result.data.id+"\")' style='color:#3292ff;cursor:pointer;'>申请开通</a>":"<a onclick='' style='cursor:pointer;'>--</a>");
				$("#isGroupHelperOperate").empty();
				$("#isGroupHelperOperate").append(result.data.isGroupHelper==0||result.data.isGroupHelper==-1?"<a onclick='Web.applicationGroupHelper(\""+result.data.id+"\")' style='color:#3292ff;cursor:pointer;'>申请开通</a>":result.data.isGroupHelper==2?"<a onclick='' style='cursor:pointer;'>--</a>":"<a onclick='Web.HelperList(\""+result.data.id+"\")' style='cursor:pointer;'>添加群助手</a>");
			}
		})
		$("#applist").hide();
		$("#createApp_div").hide();
	...
	confirmApplicationPay:function(){
		myFn.invoke({
			...
			success:function(result){
				layui.layer.alert("申请成功，待审核");
				Web.lookInfo($("#applicationPayId").val());
				$("#interfaceInfo").show();
				$("#payCallBack").hide();
			}
		})
	...
	confirmIsGroupHelper:function(){
		...
		if(param.type==2){
			...
			// if($("#downloadUrl").val()!=null&&$("#downloadUrl").val()!=""&&$("#downloadUrl").val()!=undefined){
			// 	param.downloadUrl = $("#downloadUrl").val();
			// }
			...
			param.imageUrl = $("#otherImgUrl").val();
			param.appIcon = $("#otherAppIocn").val();
		...
		myFn.invoke({
			...
			success:function(result){
				...
				Web.HelperList(param.openAppId);
		...
		myFn.invoke({
			...
			success:function(result){
				if(result.resultCode==1){
					...
					Web.HelperList($("#addHelper_openAppId").val());
				}else{
