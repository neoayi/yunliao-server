...
public class ClientConfig {
	...
	private int isOpenRegister = 1; //是否开启注册 1 开启 0 关闭

	private byte isOpenUI = 1; //是否启用客户端UI
...
//	private byte isOpenReadReceipt=1;//是否启用 已读消息回执
	 ...
	 * 1：开启 0：关闭
	 */
	private byte isOpenSecureChat=0;
	...
	private byte enableOpenSquare=1; //是否启用广场

	private byte enableAliPay = 1; // 是否启用支付宝支付
	private byte enableWxPay = 1; // 是否启用微信支付

	private InterfaceOrder interfaceOrder; //用于控制界面顺序的字段
	/**
	 * 用于接收参数，不保存数据库
	 */
	@Transient
	private  byte interfaceOrder_friend;
	@Transient
	private  byte interfaceOrder_room;
	@Transient
	private  byte interfaceOrder_company;
	...
	private String homeName;// 启动软件的首页名称
	...
	* */
	...
	public static class InterfaceOrder {
		private byte friend  = 1;   //好友
		private byte company = 3;  //同事
		private byte room = 2; //群组

		public InterfaceOrder() { }

		public InterfaceOrder( ClientConfig clientConfig ) {
			this.friend = clientConfig.getInterfaceOrder_friend();
			this.company = clientConfig.getInterfaceOrder_company();
			this.room = clientConfig.getInterfaceOrder_room();
		}
	}
}
