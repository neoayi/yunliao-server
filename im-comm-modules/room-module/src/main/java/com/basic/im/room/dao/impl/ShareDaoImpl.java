...
import com.basic.im.room.dao.ShareDao;
...
public class ShareDaoImpl extends MongoRepository<Room.Share, ObjectId> implements ShareDao {
    ...
    @Override
    public List<Room.Share> getShareList(ObjectId roomId, int userId, int pageIndex, int pageSize,long time) {
        Query query = createQuery("roomId",roomId);
        if(0 != userId) {
            addToQuery(query,"userId",userId);
        }
        if (time >= KConstants.ONE){
