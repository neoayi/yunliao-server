...
public class RoomNoticeDaoImpl extends MongoRepository<Room.Notice, ObjectId> implements RoomNoticeDao {
    ...
    @Override
    public Room.Notice getNotice(ObjectId noticeId, ObjectId roomId) {
    ...
    @Override
    public List<Room.Notice> getNoticList(ObjectId roomId, long startTime,int limit) {
        Query query = createQuery("roomId",roomId).limit(limit);
        if (KConstants.ZERO != startTime){
            query.addCriteria(Criteria.where("time").lt(startTime));
        }
        descByquery(query,"time");
        return queryListsByQuery(query);
    }
