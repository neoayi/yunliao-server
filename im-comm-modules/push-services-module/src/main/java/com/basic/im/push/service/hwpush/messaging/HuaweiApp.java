...
public class HuaweiApp {
    ...
    public static HuaweiApp initializeApp(HuaweiOption option) {
        ...
        String packageName = option.getCredential().getPackageName();
        final HuaweiApp app;
        synchronized (appsLock) {
            ...
            app.startTokenRefresher(packageName);
        }
        return app;
    }
     ...
     */
    public void startTokenRefresher(String packageName) {
        synchronized (lock) {
            checkNotDeleted();
            tokenRefresher.start(packageName);
        }
    }
