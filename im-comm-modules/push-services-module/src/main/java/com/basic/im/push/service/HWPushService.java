...
/**
 * 当前华为推送版本已弃用，新版查看com.basic.im.push.service.hwpush.examples
 * 2020-04-26 09-59
 */
//华为推送集成通知栏消息
public class HWPushService extends PushServiceUtils{
	 /*private static String appSecret = pushConfig.getHw_appSecret();
	 private static  String appId = pushConfig.getHw_appId();
	 private static  String tokenUrl = pushConfig.getHw_tokenUrl(); 
	 private static  String apiUrl = pushConfig.getHw_apiUrl();
	 private static  String iconUrl = pushConfig.getHw_iconUrl();
...
//        param.put("appPkgName", appPkgName);//定义需要打开的appPkgName
        ...
        if(!StringUtil.isEmpty(notice.getObjectId())){
            url="intent://"+pushConfig.getPackageName()+"/notification#Intent;scheme=chat;launchFlags=0x10000000;S.userId="+notice.getFrom()+";S.roomJid="+notice.getObjectId()+";end";
        }else{
            url="intent://"+pushConfig.getPackageName()+"/notification#Intent;scheme=chat;launchFlags=0x10000000;S.userId="+notice.getFrom()+";end";
        }
...
//        action.put("type", 3);//类型3为打开APP，其他行为请参考接口文档设置
        ...
        String postBody = MessageFormat.format(
         "access_token={0}&nsp_svc={1}&nsp_ts={2}&device_token_list={3}&payload={4}",
            ...
            URLEncoder.encode("openpush.message.controller.send","UTF-8"),
...
//        param.put("appPkgName", appPkgName);//定义需要打开的appPkgName
        ...
        if(null == notice.getObjectId()) {
            url="intent://"+pushConfig.getPackageName()+"/notification#Intent;scheme=chat;launchFlags=0x10000000;end";
        } else {
            url="intent://"+pushConfig.getPackageName()+"/notification#Intent;scheme=chat;launchFlags=0x10000000;S.url="+URLEncoder.encode(notice.getObjectId(), "UTF-8")+";end";
        }
        param.put("intent", url);
        JSONObject action = new JSONObject();
//        action.put("type", 3);//类型3为打开APP，其他行为请参考接口文档设置
