...
public class TokenRefresher {
     ...
     */
    final synchronized void start(String packageName) {
        ...
        logger.info("hwpush TokenRefresher start  token :{},expireIn :{}",credential.getAccessToken(),refreshDelay);
        // 华为推送自定义过期时间不准的问题
        Map<String, String> tokenMap = SKBeanUtils.getLocalSpringBeanManager().getCoreRedisRepository().getHWPushToken(packageName);
        logger.info("hwPush TokenRefresher tokenMap :{}", JSONObject.toJSONString(tokenMap));
        if(tokenMap.isEmpty()){
            credential.refreshToken();
            Map<String, String> resendTokenMap = SKBeanUtils.getLocalSpringBeanManager().getCoreRedisRepository().getHWPushToken(packageName);
            logger.info("hwPush TokenRefresher resendTokenMap :{}", JSONObject.toJSONString(resendTokenMap));
            refreshDelay = credential.getExpireIn();
            logger.info("hwPush TokenRefresher token :{},expireIn :{}",credential.getAccessToken(),refreshDelay);
        }
