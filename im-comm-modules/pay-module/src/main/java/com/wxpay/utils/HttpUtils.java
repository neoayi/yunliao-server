...
public class HttpUtils {
	...
	public static void setWxConfig(WXConfig config){
		HttpUtils.wxConfig=config;
		if(null==wxConfig) {
			System.out.println("==== 微信 支付配置 初始化失败 请检查配置文件  ");
			return;
		}
		Resource resource =null;
		try {
			if(StringUtil.isEmpty(wxConfig.getPkPath())){
				System.out.println("==== 微信 支付配置 初始化失败 请检查配置文件  ");
				return;
			}else if(StringUtil.isEmpty(wxConfig.getMchid())){
				System.out.println("==== 微信 支付配置 初始化失败 请检查配置文件  ");
				return;
			}
			if(wxConfig.getPkPath().startsWith("classpath:")) {
				resource=new ClassPathResource(wxConfig.getPkPath().replace("classpath:", ""));
				System.out.println("===============> ClassPathResource");
			}else {
				if(!StringUtil.isEmpty(wxConfig.getPkPath())){
					resource=new FileSystemResource(wxConfig.getPkPath());
			...
			char[] keyPassword =wxConfig.getMchid().toCharArray(); //证书密码
