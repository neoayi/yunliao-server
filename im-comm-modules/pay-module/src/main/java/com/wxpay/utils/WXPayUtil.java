...
public class WXPayUtil {
	...
	public static SortedMap<String, String> getPackage(WxPayDto wxPayDto,WXConfig wxConfig) {
		logger.info(wxConfig.toString());
		// 附加数据 原样返回
		//String attach = "";
		// 总金额以分为单位，不带小数点
		...
		logger.info(xml);
		String prepay_id =GetWxOrderno.getPayNo(WXPayConfig.PREPAY_ID_URL, xml);
		  ...
		  StringBuffer buffer = new StringBuffer(); 
	        ...
	        if (param != null && !param.isEmpty()) {  
	            for (Map.Entry<String, String> entry : param.entrySet()) {  
	            	if("attach".equals(entry.getKey()) || "body".equals(entry.getKey()) || "sign".equals(entry.getKey())){
