...
import com.basic.im.pay.dao.CodePayDao;
...
import com.basic.im.pay.service.PayService;
import com.basic.im.pay.utils.OtpHelper;
...
import com.basic.im.open.opensdk.OpenWebManageImpl;
...
import com.basic.im.pay.service.PayRedisService;
...
public class PayServiceImpl implements PayService {
	...
	public Integer analysisCode(String paymentCode){
		...
		String userIdCode=paymentCode.substring(userIdCodeLength.length(),Integer.parseInt(userIdCodeLength)+1);
		...
		int userId=Integer.parseInt(userIdCode)-n-Integer.parseInt(opt);
		
		long time=Integer.parseInt(timeCode)*Integer.parseInt(opt);
		if(System.currentTimeMillis()/1000-time<256){
			return userId;
		}else{
			time=Integer.parseInt(timeCode)*(Integer.parseInt(opt)-100);
			if(System.currentTimeMillis()/1000-time<256){
				return userId;
			}else{
				return null;
	...
	public  void receiptOnLock(Integer userId,Integer fromUserId,String money,String desc){
		...
		if(!StringUtil.isEmpty(desc)) {
			...
			//lessRecord.setDesc("QR code collection has been paid.");
		}
		lessRecord.setTime(DateUtil.currentTimeSeconds());
		consumeRecordDao.addConsumRecord(lessRecord);
		User sysUser = userCoreService.getUser(1100);
		...
		//addRecord.setDesc("QR code collection has been received.");
		addRecord.setTime(DateUtil.currentTimeSeconds());
		addRecord.setOperationAmount(doubleMoney);
		consumeRecordDao.addConsumRecord(addRecord);
		// 发送xmpp通知收款成功
