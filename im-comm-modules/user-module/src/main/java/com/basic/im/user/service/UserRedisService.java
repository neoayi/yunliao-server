...
public class UserRedisService extends AbstractRedisson {
     ...
     * 用户的收藏列表
     */
    public static final String USER_COLLECT_COMMON="user_collect:common:%s:%s:%s";
     ...
     * 用户实人认证
     */
    public static final String REALPERSONLICENSE = "realPersonLicense:%s";
    
    /**
     * 保存用户实人认证信息
     */
    public void saveRealPersonLicense(String userId, String realPersonInfo){
        String key = String.format(REALPERSONLICENSE, userId);
        RBucket<String> rbBucket = redissonClient.getBucket(key);
        rbBucket.set(realPersonInfo, KConstants.Expire.HALFHOUR, TimeUnit.SECONDS);
    }

    /**
     * 查询用户实人认证信息
     */
    public String queryRealPersonLicense(String userId){
        String key = String.format(REALPERSONLICENSE, userId);
     ...
     * 删除用户获取的实人认证token
     */
    public boolean deleteRealPersonLicense(String userId){
        String key = String.format(REALPERSONLICENSE, userId);
        RBucket<String> bucket = redissonClient.getBucket(key);
    ...
    public void saveUserSession(KSession session){
    ...
    public KSession queryUserSession(String accessToken){
    ...
    public void cleanUserSession(String accessToken){
        KSession session = queryUserSession(accessToken);
        if(null!=session) {
        ...
        deleteBucket(GET_SESSON_KEY, accessToken);
    }


    public static final String USER_TOKENKEYS = "loginToken:token:%s:%s";
    ...
    public boolean cleanUserAllLoginInfo(int userId){
        ...
        String token;
        KSession session;
        for (String resource : KConstants.DeviceKey.RESOURCES) {
            key=buildRedisKey(USER_TOKENKEYS,userId,resource);
            token =(String) redissonClient.getBucket(key).get();
            session = queryUserSession(token);
            if(null!=session) {
    ...
    public List<Emoji> getUserCollectCommon(Integer userId,Integer pageIndex,Integer pageSize){
        String key = String.format(USER_COLLECT_COMMON, userId,pageIndex,pageSize);
    ...
    public List<Emoji> getUserCollectCommonLimit(Integer userId,Integer pageIndex,Integer pageSize){
        ...
        return redisPageLimit(key, pageIndex, pageSize);
    }
     ...
     * @param emojis
     **/
    public void saveUserCollectCommon(Integer userId, List<Emoji> emojis,Integer pageIndex,Integer pageSize){
        String key = String.format(USER_COLLECT_COMMON,userId,pageIndex,pageSize);
