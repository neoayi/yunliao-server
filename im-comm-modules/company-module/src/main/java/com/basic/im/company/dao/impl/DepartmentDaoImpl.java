...
public class DepartmentDaoImpl extends BaseMongoRepository<Department,ObjectId> implements DepartmentDao {
    ...
    public Department modifyDepartment(Department department) {
        ...
        if (null != department.getDepartmentDirector()){
            ops.set("departmentDirector",department.getDepartmentDirector());
        }
    ...
    @Override
    public Department queryRootDepartment(ObjectId companyId) {
        Query query = createQuery("companyId",companyId);
        query.addCriteria(Criteria.where("type").is(1));  // 1 : 根部门
        return findOne(query);
    }
