...
public interface EmployeeDao extends IBaseMongoRepository<Employee,ObjectId> {
    ...
    //部门是否存在员工数据
    boolean departIsExistsEmployeeData(ObjectId departmentId);

    //员工是否存在
    boolean employeeIsExists(ObjectId companyId, int userId);
    ...
    //公司人数
    long calculateEmployeeNumber(ObjectId companyId);

    List<Integer> queryEmployeeUserIdList(ObjectId companyId);

    ObjectId queryEmployeeCompanyId(Integer userId);
}
