...
public interface CompanyManager {
	...
	//部门列表（包括员工数据）
	List<Department> departmentList(ObjectId companyId, byte isReturnEmployData);
	...
	// 修改员工部门
	Employee updateEmpDeft(Employee employee);

    List<Integer> queryEmployeeUserIdList(ObjectId companyId);

    ObjectId queryCompanyId(Integer userId);
	...
	*/
	...
	//Excel导入数据，存员工的信息到员工表
	ObjectId importEmployee(Employee employee);

	//Excel导入数据，存员工的部门信息到部门表

	//此方法用于将客户自动加入默认的公司便于，客户体验组织架构功能
	Company autoJoinCompany(int userId);


    void delCompany(Integer userId);
}