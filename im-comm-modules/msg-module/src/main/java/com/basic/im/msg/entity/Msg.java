...
public class Msg {
	...
	@Indexed
	private Integer visible=1; //是否可见（1：好友可见，2：私密，3：部分可见选中的朋友的可见，4：不给谁看，5:公开陌生人可见 ）
	...
	@Transient
	private  List<Givegift> gifts;// 礼物列表

	@Transient
	private  List<Praise> praises;// 赞列表
	@Transient
	private  List<ForwardAmount> forwardAmount;// 转发列表
	@Transient
	private  List<PlayAmount> playAmount;// 播放列表
	@Transient
	private  int isPraise;// 请求接口人是否赞过本条消息  0：未点赞 1：已点赞
	@Transient
	private int isCollect;// 请求接口人是否收藏本条消息 0：未收藏过 1：已收藏
	private int state;// 是否被锁定  0：正常  1：锁定  2：未审核 -1：审核失败
	...
	public static Msg build(User user, AddMsgParam param) {
		...
		if(10000==user.getUserId()) {
            ...
            //entity.nickname ="Customer service public account";
        } else {
		...
		entity.body = body;
		if(!StrUtil.isEmpty(param.getLables())){
			entity.lables=StrUtil.split(param.getLables(),"#",-1,false,true);
		}
		entity.count = count;
		...
		entity.setLoc(new Msg.Loc(param.getLongitude(), param.getLatitude()));
