...
//                throw new ServiceException(KConstants.ResultCode.DontVisitFriendsMsg);
    ...
    public List<Msg> findByUser(Integer userId,Integer toUserId, ObjectId msgId,int pageIndex, Integer pageSize) {
        ...
        Criteria criteria5 = Criteria.where("visible").is(5);

        Criteria criteria=createCriteria().orOperator(criteria1,criteria2,
...
//    }
    ...
    public List<Msg> queryPublicMsg(PublicMsgQueryModel queryModel){
        Query query = createQuery();
        //User user=userCoreService.getUser(queryModel.getRequestUserId());
        /*if(null!=user.getSettings()&&null!=user.getSettings().getFilterCircleUserIds()) {
            query.addCriteria(Criteria.where("userId").nin(user.getSettings().getFilterCircleUserIds()));
        }*/
        query.addCriteria(Criteria.where("state").is(0));
        if(!StringUtil.isEmpty(queryModel.getLable())) {
            //query.addCriteria(containsIgnoreCase("body.lable",queryModel.getLable()));
            //query.addCriteria(Criteria.where("body.lable").is(lable));
            query.addCriteria(Criteria.where("lables").in(queryModel.getLable()));
        }

        query.addCriteria(Criteria.where("visible").is(5));

        if(0!=queryModel.getCityId()) {
            query.addCriteria(Criteria.where("cityId").is(queryModel.getCityId()));
        }


        /*
        List<Integer> users=new ArrayList<Integer>();
        users.add(queryModel.getRequestUserId());
        Criteria criteria1 = Criteria.where("visible").is(1).and("userNotLook").nin(users);
        Criteria criteria2=Criteria.where("userId").is(queryModel.getRequestUserId());
        Criteria criteria3=Criteria.where("visible").is(3).and("userLook").in(users);
        Criteria criteria4=Criteria.where("visible").is(4).and("userNotLook").nin(users);
        Criteria criteria=createCriteria().orOperator(criteria1,criteria2,criteria3,criteria4);
        query.addCriteria(criteria);*/

        /* query.addCriteria(Criteria.where("loc").nearSphere(
                           new Point(queryModel.getLongitude(), queryModel.getLatitude())
                    ));*/
            /*query.addCriteria(Criteria.where("loc").nearSphere(
                    new Point(queryModel.getLongitude(), queryModel.getLatitude())
            ));*/

        /*if(0 != queryModel.getLatitude() && 0 != queryModel.getLongitude()) {
            if(0==queryModel.getDistance()){
                queryModel.setDistance(100);
            }
            query.addCriteria(Criteria.where("loc").withinSphere(
                    new Circle(new Point(queryModel.getLongitude(),queryModel.getLatitude()), new Distance(queryModel.getDistance(), Metrics.KILOMETERS)))
            );
        }*/

        if(!StrUtil.isEmpty(queryModel.getKeyword())){
            Criteria criteria = createCriteria().orOperator(
                    containsIgnoreCase("body.text",queryModel.getKeyword()),
                    containsIgnoreCase("body.title",queryModel.getKeyword())
            );
            query.addCriteria(criteria);
        }

        //String sortKey="time";
        switch (queryModel.getSortType()){
            case 1:
                descByquery(query,"time");
                //descByquery(query,"loc");
            default:
                descByquery(query,"time");
                //descByquery(query,"loc");
                //sortKey="time";
        ...
        return queryListsByQuery(query,queryModel.getPageIndex(),queryModel.getPageSize());
    }
    ...
    public List<Msg> getPureVideo(int userId,Integer pageIndex,Integer pageSize,String lable){
        ...
        query.addCriteria(Criteria.where("state").ne(1));
        //query.addCriteria(Criteria.where("visible").ne(2).and("userId").ne(userId));
        query.addCriteria(Criteria.where("body.videos").ne(null));
        if(!StringUtil.isEmpty(lable) && "8".equals(lable)) {
            query.addCriteria(Criteria.where("lables").is(null));
        } else if(!StringUtil.isEmpty(lable)) {
            //query.addCriteria(containsIgnoreCase("body.lable",lable));
            query.addCriteria(Criteria.where("lables").is(lable));
        }
    ...
    @Override
    public PageResult<Msg> getMsgListResult(int userId, String nickName, int pageIndex, int pageSize,String type) {
        ...
        if (!StringUtil.isEmpty(type)){
            query.addCriteria(Criteria.where("body.type").is(Integer.valueOf(type)));
        }
        descByquery(query,"time");
