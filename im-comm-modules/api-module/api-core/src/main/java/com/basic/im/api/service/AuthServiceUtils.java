package com.basic.im.api.service;

import com.alibaba.fastjson.JSONObject;
import com.alibaba.fastjson.TypeReference;
import com.basic.im.comm.utils.LoginPassword;
import com.basic.im.common.MoneyChangeUtil;
...
import com.basic.im.user.service.IBaseUserManager;
...
import com.basic.utils.ParamsSign;
...
public class AuthServiceUtils {
    ...
    public boolean authRequestApiByMac(Map<String, String> paramMap, KSession session, String url) {
        if (null == session) {
        ...
        if (!checkMacSignBase64(mac, macStrBuf.toString(), session.getHttpKey())) {
            logger.info("checkMacSignBase64 is false");
            return false;
        }
        if (checkMacExistRedis(session.getUserId(), url + mac)) {
            logger.error("重复请求 userId {} url {} mac {}", session.getUserId(), url, mac);
            return false;
        } else {
            return true;
        }
        //return !checkMacExistRedis(session.getUserId(),url+mac);
    }
    ...
    public JSONObject authJoinRoom(int userId, String token, String data, String codeId, String payPwd) {
        byte[] decode = getPayCodeById(userId, codeId);
        if (null == decode) {
            return null;
        }
        ...
        StringBuffer macStrBuf = new StringBuffer();
        macStrBuf.append(getApiKey()).append(userId).append(token);
        macStrBuf.append(jsonObj.get("roomId")).append(jsonObj.get("price")).append(jsonObj.get("periods"));
