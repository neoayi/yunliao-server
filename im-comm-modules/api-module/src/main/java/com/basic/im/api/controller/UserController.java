...
import com.basic.im.user.entity.User.DeviceInfo;
...
public class UserController extends AbstractController {
    ...
    public JSONMessage register(@Valid UserExample example, HttpServletRequest request) {
        try {
            ...
            if( 0 == SKBeanUtils.getImCoreService().getClientConfig().getIsOpenRegister() ){
                return JSONMessage.failureByErrCode(ResultCode.NoRegistrationFunctionIsProvided);
            }
            ...
            if (StringUtil.isEmpty(example.getTelephone())) {
                ...
                return JSONMessage.failureByErrCode(ResultCode.PleaseNickName);
            } else if (userManager.checkUserNameNotwords(example.getNickname())) {
                return JSONMessage.failureByErrCode(ResultCode.NicknameWrongful);
            }
    ...
    public JSONMessage sdkLogin(@ModelAttribute LoginExample example, @RequestParam int type, @RequestParam String loginInfo, HttpServletRequest request) {
        SdkLoginInfo sdkLoginInfo = userManager.findSdkLoginInfo(type, loginInfo);
        if (sdkLoginInfo != null && null != sdkLoginInfo.getUserId()) {
    ...
    public JSONMessage qrCodeLogin(@RequestParam String qrCodeKey, @RequestParam int type,@RequestParam(defaultValue = "") String userKey) {
    ...
    public JSONMessage loginAuto(HttpServletRequest request, @ModelAttribute LoginExample example, @RequestParam(defaultValue = "") String access_token) {
    ...
    public JSONMessage updateUser(@ModelAttribute @ApiParam UserExample param) {
        try {
            ...
            if (userManager.isOpenMultipleDevices(userId)) {
                MultipointSyncUtil.multipointLoginUpdateUserInfo(userId, userManager.getUser(userId).getNickname(), null, null, 0);
            }
            return JSONMessage.success(data);
        } catch (ServiceException e) {
            return JSONMessage.failureByException(e);
    ...
    public JSONMessage changeMsgNum(@RequestParam int num) {
        userCoreRedisRepository.changeMsgNum(ReqUtil.getUserId(), num);
        return JSONMessage.success();
    }
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "query", name = "packageName", value = "包名", dataType = "String"),
            @ApiImplicitParam(paramType = "query", name = "regId", value = "编号", dataType = "String")
    })
    @RequestMapping("/jPush/setRegId")
    public JSONMessage setJPushRegId(@RequestParam(defaultValue = "com.chat.weichat") String packageName, String regId) {
        ...
        info.setLanguage(ReqUtil.getRequestLanguage());
    ...
    public JSONMessage setJPushIOSRegId(@RequestParam(defaultValue = "com.chat.weichat") String appId, String regId) {
        if (StringUtil.isEmpty(regId)) {
            return JSONMessage.success();
        }
        Integer userId = ReqUtil.getUserId();
    ...
    public JSONMessage setRegId(@RequestParam(defaultValue = "com.sk.weichat") String packageName, String regId) {
        if (StringUtil.isEmpty(regId)) {
            return JSONMessage.success();
        }
        Integer userId = ReqUtil.getUserId();
        ...
        info.setLanguage(ReqUtil.getRequestLanguage());
        info.setPackName(packageName);
        int oldUserId = userManager.savePushToken(userId, info);
        if (0 != oldUserId) {
            userCoreRedisRepository.removeAndroidPushToken(oldUserId);
        }
        userCoreRedisRepository.saveAndroidPushToken(userId, info);
        return JSONMessage.success();
    }
    ...
    public JSONMessage setApnsToken(@RequestParam(defaultValue = "") String deviceId, String token,
        ...
        int oldUserId = userManager.savePushToken(userId, info);
        if (0 != oldUserId) {
            userCoreRedisRepository.removeIosPushToken(oldUserId);
        }
        return JSONMessage.success();
    }
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "query", name = "packageName", value = "包名", dataType = "String", defaultValue = "0"),
            @ApiImplicitParam(paramType = "query", name = "token", value = "token值", dataType = "String"),
            @ApiImplicitParam(paramType = "query", name = "adress", value = "地址", dataType = "String")
    })
    @RequestMapping("/hwpush/setToken")
    public JSONMessage setHWToken(String token, String adress, @RequestParam(defaultValue = "com.sk.weichat") String packageName) {
        if (StringUtil.isEmpty(token)) {
            return JSONMessage.failure("null Token");
        }
        DeviceInfo info = new DeviceInfo();
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(paramType = "query", name = "token", value = "包名", dataType = "String")
    })
    @RequestMapping("/fcmPush/setToken")
    public JSONMessage setFCMToken(@RequestParam(defaultValue = "") String token, @RequestParam(defaultValue = "com.sk.weichat") String packageName) {
        if (StringUtil.isEmpty(token)) {
            return JSONMessage.failure("null Token");
        }
        DeviceInfo info = new DeviceInfo();
     ...
     * @return
     */
    @ApiOperation("清除客户端推送token")
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "query", name = "deviceKey", value = "设备号", dataType = "String")
    })
    @RequestMapping("/deletePushToken")
    public JSONMessage deletePushToken(@RequestParam(defaultValue = "android") String deviceKey,
                                       @RequestParam(defaultValue = "com.sk.weichat") String packageName) {
    ...
    public JSONMessage setMZPushId(@RequestParam(defaultValue = "") String pushId, @RequestParam(defaultValue = "com.sk.weichat") String packageName) {
    ...
    public JSONMessage setVIVOPushId(@RequestParam(defaultValue = "") String pushId, @RequestParam(defaultValue = "com.sk.weichat") String packageName) {
        if (StringUtil.isEmpty(pushId)) {
            return JSONMessage.failure("null pushId");
        }
        DeviceInfo info = new DeviceInfo();
    ...
    public JSONMessage setOPPOPushId(@RequestParam(defaultValue = "") String pushId, @RequestParam(defaultValue = "com.sk.weichat") String packageName) {
        if (StringUtil.isEmpty(pushId)) {
            return JSONMessage.failure("null pushId");
        }
        DeviceInfo info = new DeviceInfo();
    ...
    @ApiOperation("获取用户列表")
    @ApiImplicitParam(paramType = "query", name = "userIds", value = "用户编号列表", dataType = "String", defaultValue = "")
    @RequestMapping(value = "/get/list")
    public JSONMessage getUserList(@RequestParam(name = "userIds",defaultValue = "") List<Integer> userIds) {
        VerifyUtil.isEmpty(userIds);
        Map<String,User> resultMap = new HashMap<>();
        List<User> userList = userManager.findUserByIds(userIds);
        userList.forEach(obj->{
            wrapUser(obj,obj.getUserId());
            //查找用户公私钥
            Optional<AuthKeys> authKeys = Optional.ofNullable(authKeysService.getAuthKeys(obj.getUserId()));
            if (authKeys.isPresent()) {
                obj.setDhMsgPublicKey((authKeys.get().getMsgDHKeyPair() != null) ? authKeys.get().getMsgDHKeyPair().getPublicKey() : null);
                obj.setRsaMsgPublicKey((authKeys.get().getMsgRsaKeyPair() != null) ? authKeys.get().getMsgRsaKeyPair().getPublicKey() : null);
            }
            resultMap.put(String.valueOf(obj.getUserId()),obj);
        });
    ...
    public JSONMessage getUser(@RequestParam(defaultValue = "") String userId) {
        try {
            ...
            wrapUser(user,toUserId);
            Friends friends = friendsManager.getFriends(ReqUtil.getUserId(), toUserId);
            if (null != friends){
                //他对我设置的
                user.setShowMarker(friends.getToShowMarker() == null ? "" : friends.getToShowMarker());
                //我对他设置的
                user.setToShowMarker(friends.getShowMarker() == null ? "" : friends.getShowMarker());
            }
            return JSONMessage.success(user);
        } catch (ServiceException e) {
            return JSONMessage.failureByException(e);
     ...
     * 包装 user 实体对象返回值
     */
    private void wrapUser(User user,Integer toUserId){
        user.setOnlinestate(userCoreRedisRepository.queryUserOnline(toUserId)?1:0);
        ...
        if (!ReqUtil.getUserId().equals(toUserId)) {
            // 用户获取他人的用户信息
            ...
            if(authKeys.isPresent()) {
                user.setPayPassword(StringUtil.isEmpty(authKeys.get().getPayPassword()) ? "0" : "1");
            }else {
        ...
        // 用户是否开启云钱包功能
        //user.setWalletUserNo(!StringUtil.isEmpty(authKeysService.getWalletUserNo(user.getUserId()))? KConstants.ONE : KConstants.ZERO);
        user.setWalletId(authKeysService.getUpayWalletId(user.getUserId()));
        if (user.getUserId().equals(10000)) {
            Locale requestLocale = LocaleMessageUtils.getRequestLocale();
            user.setNickname(LocaleMessageUtils.getMessage("serviceNo", requestLocale));
        }
    }
    ...
    public JSONMessage getUserByAccount(@RequestParam(defaultValue = "") String account) {
        try {
            ...
            try {
                if (null == user) {
                    user = userManager.getUser(loginedUserId, Integer.parseInt(account));
                }
            } catch (Exception e) {
...
//					userManager.savaDeviceMessage(user.getUserId(), serial, deviceType);
            ...
            if(authKeys!=null && authKeys.getMsgDHKeyPair()!=null) {
    ...
    public JSONMessage updatePassword(@RequestParam("oldPassword") String oldPassword,
        ...
        try {
            if (StringUtil.isEmpty(oldPassword) || StringUtil.isEmpty(newPassword)) {
                ...
                userRedisService.cleanUserSession(getAccess_token());
                userCoreRedisRepository.deleteUserByUserId(userId);
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(paramType = "query", name = "fileSize", value = "文件大小", dataType = "String", required = true)
    })
    ...
    public JSONMessage sendMessage(@RequestParam(defaultValue = "") String jid,
            ...
            , @RequestParam(defaultValue = "") String content, @RequestParam(defaultValue = "") String fileName, @RequestParam(defaultValue = "0") int fileSize) {
        if (StringUtil.isEmpty(jid) || StringUtil.isEmpty(content)) {
            return JSONMessage.success();
        }
        userManager.sendMessage(jid, chatType, type, content, fileName, fileSize);
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(paramType = "query", name = "reportType", value = "举报类型", dataType = "int"),
            @ApiImplicitParam(paramType = "query", name = "reportInfo", value = "举报详情(eg:朋友圈的id)", dataType = "String")
    })
    @RequestMapping("/report")
    public JSONMessage report(@RequestParam(defaultValue = "0") Integer toUserId, @RequestParam(defaultValue = "") String roomId,
                              @RequestParam(defaultValue = "") String webUrl, @RequestParam(defaultValue = "0") int reason, @RequestParam(defaultValue = "0") int reportType,
                              @RequestParam(defaultValue = "") String reportInfo) {
        try {
            userManager.report(ReqUtil.getUserId(), toUserId, reason, roomId, webUrl, reportType, reportInfo);
        } catch (Exception e) {
    ...
    public JSONMessage checkReportUrl(@RequestParam(defaultValue = "") String webUrl, HttpServletResponse response) throws IOException {
        ...
        if (StringUtil.isEmpty(webUrl)) {
            ...
            try {
                ...
                byte weburlType = webUrlRosterManager.checkWebUrlType(webUrl);
                if (-1 != weburlType ){
                    // weburlType  0 :  既不是黑名单也不是白名单   1 : 白名单
                    JSONObject successResult = new JSONObject();
                    successResult.put("urlType",weburlType);
                    return JSONMessage.success(successResult);
                }else { //黑名单
                    return JSONMessage.failureByErrCode(ResultCode.WEBURLISREPORTED);
                }
    ...
    public JSONMessage addEmoji(@RequestParam(defaultValue = "") String emoji,
                                @RequestParam(defaultValue = "") String url,
                                @RequestParam(defaultValue = "") String roomJid,
                                @RequestParam(defaultValue = "") String msgId,
    ...
    public JSONMessage emojiList(@RequestParam Integer userId,
    ...
    public JSONMessage collectionList(@RequestParam Integer userId,
    ...
    public JSONMessage searchCollection(@RequestParam(name = "userId",    defaultValue = "0")   Integer userId,
                                        @RequestParam(name = "keyword",   defaultValue = "")    String keyword,
                                        @RequestParam(name = "pageIndex", defaultValue = "0")   int pageIndex,
                                        @RequestParam(name = "pageSize",  defaultValue = "10")  int pageSize) {
        Object data = collectService.searchCollection(userId, keyword, pageSize, pageIndex);
        return JSONMessage.success(data);
    }

    @ApiOperation("取消收藏")
    @ApiImplicitParam(paramType = "query", name = "emojiId", value = "收藏Id，删除多个传递使用 , 分割", dataType = "String", required = true)
    @RequestMapping("/emoji/delete")
    public JSONMessage deleteEmoji(@RequestParam String emojiId) {
        collectService.deleteEmoji(VerifyUtil.verifyUserId(ReqUtil.getUserId()), emojiId);
        return JSONMessage.success();
    }
...
//		}
    ...
    @ApiOperation("实名认证")
    @RequestMapping("/realNameCertified")
    public JSONMessage realNameCertified() {
        try {
            userManager.identityVerifieFlagImpl(ReqUtil.getUserId());
            return JSONMessage.success();
        } catch (ServiceException e) {
            e.printStackTrace();
            return JSONMessage.failureByException(e);
    ...
    @ApiOperation("取消实名认证")
    @RequestMapping("/cancelRealNameCertified")
    public JSONMessage cancelRealNameCertified() {
        try {
            userManager.cancelRealNameCertifiedImpl(ReqUtil.getUserId());
            return JSONMessage.success();
        } catch (ServiceException e) {
            return JSONMessage.failureByException(e);
        }
    ...
    @ApiOperation("实人认证发起认证请求，获取认证Token")
    @RequestMapping("/getDescribeVerifyToken")
    public JSONMessage getDescribeVerifyToken() {
        try {
            String describeVerifyToken = userManager.getDescribeVerifyToken(ReqUtil.getUserId());
//			Object parse = JSONObject.parse(describeVerifyToken);
            JSONObject parse = JSONObject.parseObject(describeVerifyToken, JSONObject.class);
            return JSONMessage.success(parse);
        } catch (Exception e) {
            return JSONMessage.failureByException(e);
        }
    }
...
//	}
    ...
    @ApiOperation("更新地理位置")
    @RequestMapping("/modity/location")
    public JSONMessage modityLocation(@Valid LocationExample locationExample) {
        Integer userId = ReqUtil.getUserId();
        userManager.modityLocation(userId, locationExample);
        return JSONMessage.success();
    }
}
