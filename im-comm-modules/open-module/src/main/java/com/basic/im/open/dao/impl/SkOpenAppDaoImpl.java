package com.basic.im.open.dao.impl;
...
public class SkOpenAppDaoImpl extends MongoRepository<SkOpenApp, ObjectId> implements SkOpenAppDao {
    ...
    @Override
    public List<SkOpenApp> applicationList(String accountId,int appType,int pageIndex,int pageSize, String keyword) {
        Query query;
        if (StringUtil.isEmpty(accountId)){
             query=createQuery();
        }else{
             query=createQuery("accountId",accountId);
        }
        if (!StringUtil.isEmpty(keyword)){
            query.addCriteria(Criteria.where("appName").regex(keyword));
        }
