...
import com.basic.im.comm.utils.DesUtil;
import com.basic.im.message.MessageService;
import com.basic.im.open.dao.GroupHelperDao;
import com.basic.im.open.dao.HelperDao;
import com.basic.im.open.dao.OpenLoginInfoDao;
import com.basic.im.open.dao.SkOpenAppDao;
import com.basic.im.open.entity.GroupHelper;
...
import com.basic.im.open.opensdk.until.SkOpenUtil;
...
public class OpenAppManageImpl{
	 ...
	 * @return
	 */
	public List<SkOpenApp> applicationList(String userId,Integer type,Integer pageIndex,Integer pageSize,String keyword){
		return skOpenAppDao.applicationList(userId,type,pageIndex,pageSize,keyword);
	}
...
//		String key = time.substring(2, time.length());
		...
		String encodeBuffer = Base64.getEncoder().encodeToString(str.getBytes());  
		return encodeBuffer;
	}
	...
	public Map<String,String> codeOauthImpl(String code) throws Exception {
		...
		byte[] decodeBuffer = Base64.getDecoder().decode(code);  
		...
		// des解密userId
		User user = userCoreService.getUser(Integer.parseInt(DesUtil.decrypt(desUserId, codeTime)));
		if(null == user) {
		...
		if(null == openLoginInfo){
			...
			openId = Base64.getEncoder().encodeToString(openIdByte.getBytes());// base64
