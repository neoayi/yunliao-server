...
public class StringUtils {
     ...
     */
    private static final String PREFIX = StringUtils.randomCode() + "-";
    ...
    public static String randomCode() {
		...
		return "" + (random.nextInt(899999) + 1000000);
	}
...
public static String listToString(Collection<String> list,String splitStr){
   ...
   return result.toString();
	 ...
	 * @return
	 */
	public static StringBuffer getMethoedPath(){
		StringBuffer buffer = new StringBuffer();
//        buffer.append("方法调用路线 ");
		StackTraceElement[] stackTrace = Thread.currentThread().getStackTrace();
		for(int i = stackTrace.length-1; i>0; i--){
			if("invoke".equals(stackTrace[i].getMethodName())){
				continue;
			}
			if(i!= stackTrace.length-1&&i> 1){
				buffer.append("->");
			}
			if(i> 1){
