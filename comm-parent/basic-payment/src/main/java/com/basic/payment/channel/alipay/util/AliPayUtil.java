...
import com.basic.payment.channel.alipay.sign.RSA;

import java.io.UnsupportedEncodingException;


public class AliPayUtil {
	static AliPayProperties aliPayConfig=null;

	public static void setAppConfig(AliPayProperties config){
		AliPayUtil.aliPayConfig=config;
		APP_ID=config.getAppid();
		APP_PRIVATE_KEY=config.getApp_private_key();
		ALIPAY_PUBLIC_KEY=config.getAlipay_public_key();
...
//			model.setGoodsType("0");
	...
	public static CertAlipayRequest getClientParams(AliPayProperties properties) {
		CertAlipayRequest certParams = new CertAlipayRequest();
		certParams.setServerUrl(ALIPAY_GATEWAY);
		//请更换为您的AppId
		certParams.setAppId(properties.getAppid());
		//请更换为您的PKCS8格式的应用私钥
		certParams.setPrivateKey(properties.getApp_private_key());
		certParams.setAlipayPublicCertContent(properties.getAlipay_public_key());
	 ...
	 public static String sign(String content) {
	      return RSA.sign(content, APP_PRIVATE_KEY,CHARSET);
	   }
