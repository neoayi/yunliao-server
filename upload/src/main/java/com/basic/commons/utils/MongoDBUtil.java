   ...
   /* static {
        System.out.println("===============MongoDBUtil初始化========================");
	     CompositeConfiguration config = new CompositeConfiguration();
	        ...
	        } catch (ConfigurationException process) {
    ...
    public static MongoClient getMongoClient(String urIStr){
        ...
        // options.maxAutoConnectRetryTime(10); // the maximum auto connect retry time
        options.connectionsPerHost(300);// 连接池设置为300个连接,默认为100
        options.connectTimeout(200000);// 连接超时，推荐>3000毫秒
        options.maxWaitTime(200000); //
        options.socketTimeout(200000);// 套接字超时时间，0无限制
        options.threadsAllowedToBlockForConnectionMultiplier(5000);// 线程队列数，如果连接线程排满了队列就会抛出“Out of semaphores to get db”错误。
        options.writeConcern(WriteConcern.SAFE);//
        options.build();
        return mongoClient;
    ...
    public static DBCollection getDB(MongoClient mongoClient,String dbName,String tbName) {
        if (dbName != null && !"".equals(dbName)) {
           ...
           if(!StringUtils.isEmpty(tbName)&&null!=database)
