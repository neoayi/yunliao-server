...
public  class ConfigUtils {
	...
	*/
	public static SystemConfig getSystemConfig() {
	 ...
	 public static String configInfo() {
		    return "{"
			  ...
			  + "\n  delFileUri = " + getSystemConfig().getDelFileUri()
			  ...
			  + "\n  thumbnailSize = " + getSystemConfig().getThumbnailSize()
	...
	/*public static String getAvatarFileName(String userId){
		String fileName= Md5Utils.md5Hex(Base64.encode(userId));
		return Joiner.on("").join(fileName, ".", "jpg");
	}*/
	public static String getAvatarFileName(String userId){
		return Joiner.on("").join(userId, ".","jpg" );
	}


	public static List<File> getAvatarLocalPath(List<Long> userIdList) {
		...
		for (Long userId : userIdList) {
			 subName = String.valueOf(userId % 10000);

			 oPath = new File(getBasePath()+"/avatar/o/"+subName+"/"+userId+".jpg");
			 if(!oPath.exists()){
				 if(null!=avatarDef){
				 	fileList.add(avatarDef);
				 	continue;
				 }
				 try {
					 ClassPathResource classPathResource = new ClassPathResource(getDefultAvatarLocalPath());

					 avatarDef =File.createTempFile("avatar","png");
					 try {
						 org.apache.commons.io.FileUtils.copyInputStreamToFile(classPathResource.getInputStream(), avatarDef);
					 } finally {
	...
	public static String getDefultAvatarLocalPath(){
		return "avatar.png";
	}
	...
	public static File[] getGroupAvatarPath(String jid) {
		...
		//avatar_group
		File oPath = new File(getBasePath()+"/avatar_group/o/"+oneLevelName+"/"+twoLevelName);
		File tPath = new File(getBasePath()+"/avatar_group/t/"+oneLevelName+"/"+twoLevelName);
		if (!oPath.exists())
			oPath.mkdirs();
		if (!tPath.exists())
			tPath.mkdirs();
	...
	public static File[] getCategoryPath() {

		File oPath = new File(getBasePath()+"/mall/category");
		if (!oPath.exists())
			oPath.mkdirs();
	...
	public static String getUrl(String path) {
		if(1==getSystemConfig().getIsBackDomain()) {
			if(1==getSystemConfig().getIsOpenfastDFS()) { //启用fastDfs
				return Joiner.on("").join(getSystemConfig().getFastdfsDomain(), path.replace('\\', '/')).replaceFirst(getSystemConfig().getFastdfsBasePath(), "");
			}
			return Joiner.on("").join(getSystemConfig().getDomain(),path.substring(getSystemConfig().getBeginIndex())).replace('\\', '/');
		}else {
			if(1==getSystemConfig().getIsOpenfastDFS()) { //启用fastDfs
				return Joiner.on("").join("", path.replace('\\', '/')).replaceFirst(getSystemConfig().getFastdfsBasePath(), "");
			}
	...
	public static String getUrl(File file) {
		if(1==getSystemConfig().getIsBackDomain()) {
			if(1==getSystemConfig().getIsOpenfastDFS()) { //启用fastDfs
				return Joiner.on("").join(getSystemConfig().getFastdfsDomain(), file.getPath().replace('\\', '/')).replaceFirst(getSystemConfig().getFastdfsBasePath(), "");
			}
			return Joiner.on("").join(getSystemConfig().getDomain(), file.getPath().substring(getSystemConfig().getBeginIndex())).replace('\\', '/');
		}else {
			if(1==getSystemConfig().getIsOpenfastDFS()) { //启用fastDfs
				return Joiner.on("").join("", file.getPath().replace('\\', '/')).replaceFirst(getSystemConfig().getFastdfsBasePath(), "");
			}
	...
	public static String getFullUrl(String path) {
		...
		return (getSystemConfig().getDomain() + path.substring(getSystemConfig().getBeginIndex())).replace('\\', '/');
	}
	...
	public static String getAvatarUrl(String path) {
		return  Joiner.on("").join(getSystemConfig().getFastdfsDomain(), path.replace('\\', '/')).replaceFirst(getSystemConfig().getBasePath(), "");
	}
	...
	public static String getFilePath(String url) {
		...
		childPath=getFormatUrl(childPath); // 去除多余的 / 进行匹配
		if (childPath.startsWith("/files/") || childPath.startsWith("files/")){
			childPath=childPath.replaceFirst("files","");
		}
